...
void dirwindow_autoupdate( void )
{
	...
	for	( wnr = 1,pw = fenster+1; wnr <= ANZFENSTER; wnr++,pw++)
		...
		err = Fxattr( 0, w->path, &xa );
		if	((!err) && (
				(xa.st_mtim.u.d.time != w->mtime) || (xa.st_mtim.u.d.date != w->mdate)
					)
			)
			{
			...
			if	(read_wind(w, FALSE))
				...
				if	(old_sel[0])
					...
					for	(j = 0; j < old_realnum;
							j++,old_pmydta++,old_pobj++)
						{
