...
DIALOG *OpenDialog(HNDL_OBJ proc,OBJECT *tree,char *title,int x, int y)
{
	...
	if(ptr)
	{
		...
		{
		int msg[8]={WM_TOPPED,0,0,0,0,0,0,0};
			msg[3]=ptr->whandle;
			msg[1]=ap_id;
		...
		{
		int msg[8]={WM_UNICONIFY,0,0,0,0,0,0,0};
			msg[3]=ptr->whandle;
		...
		GRECT big,small={0,0,0,0};
			...
			whandle=wdlg_open(ptr->dial,ptr->title,CLOSER|MOVER|NAME|SMALLER,ptr->last.g_x,ptr->last.g_y,0,NULL);
			if(whandle)
			{
				ptr->whandle=whandle;
				ptr->status|=WIS_OPEN;
	...
	int whandle;
		...
		if(ptr->dial)
		{
		int kind=CLOSER|MOVER|NAME|SMALLER;
		GRECT big,small={0,0,0,0};
			...
			GRECT screen,wout;
				...
				wout.g_x=min(max(wout.g_x,screen.g_x),screen.g_x+screen.g_w-wout.g_w);
				wout.g_y=min(max(wout.g_y,screen.g_y),screen.g_y+screen.g_h-wout.g_h);
...
void SendCloseDialog(DIALOG *dial)
{
int msg[8]={WM_CLOSED,0,0,0,0,0,0,0};
	msg[1]=ap_id;
