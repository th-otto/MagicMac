...
WORD cdecl hdl_anwndg(struct HNDL_OBJ_args args)
{
	...
	if (args.obj == HNDL_INIT)
	{
		...
		ltoa(mypgm.memlimit, tree[LIMITMEM].ob_spec.tedinfo->te_ptext, 10);
	...
	if (args.obj == ANWND_FS)			/* Dateiauswahl */
	{
		...
		if (mypgm.config != old)
		{
			...
			objs_setradio(tree, ex, ANWND_PR, ANWND_TO, ANWND_TP, 0);
