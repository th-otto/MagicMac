...
static int ctrl_objs1[NLINES] = { PN_BK, PN_UP, PN_DOWN, PN_BSL, PN_SLID };
static int objs1[NLINES] = { PRG1, PRG2, PRG3, PRG4, PRG5 };
static int ctrl_objs2[NLINES] = { DN_BK, DN_UP, DN_DOWN, DN_BSL, DN_SLID };
static int objs2[NLINES] = { DAT1, DAT2, DAT3, DAT4, DAT5 };
  ...
  found:
	if (dat->pgm == selected_pgmnr)
	{
		for (k = 0; k < selected_program->ntypes; k++)
		{
			...
			{
				lbox_ascroll_to(sbox2, k, NULL, NULL);
				if (d)
...
static void cdecl select_item1(struct SLCT_ITEM_args args)
{
...
static WORD cdecl set_item1(struct SET_ITEM_args args)
{
	struct zeile *line = (struct zeile *) args.item;
	GRECT *redr;
	...
	if (sbox2 && !selected_program)
	{
		...
		if (len != lbox_get_first(sbox2))
		{
			if (d_ica)
				...
				redr = NULL;
			lbox_ascroll_to(sbox2, len, redr, redr);
		}
	}


	if ((!line) || ((line->reldatnr != -1) && (line->reldatnr != 0)))
	{
...
static WORD cdecl set_item2(struct SET_ITEM_args args)
{
	struct zeile *line = (struct zeile *) args.item;
	struct spd_file *sd = (struct spd_file *) args.item;
	...
	if ((!line) || (!line->selected))
		args.tree[args.obj_index].ob_state &= ~SELECTED;

	if (len == -1)
		args.tree[args.obj_index].ob_flags &= ~TOUCHEXIT;
	else
		args.tree[args.obj_index].ob_flags |= TOUCHEXIT;
	...
	dob = args.tree + args.obj_index;
	dob->ob_flags &= ~HIDETREE;
	strncpy(dob->ob_spec.tedinfo->te_ptext + 1, mydat->name, visible_datlen);
/*	len = (int) strlen(dob -> ob_spec.tedinfo->te_ptext+1); */
	if (line->selected)
		ob_sel(dob, 0);
	else
...
/*	int dummy;	*/
	...
	sbox1 = lbox_create(adr_ica_dialog, select_item1, set_item1, sc,	/* Items */
						5,				/* Anzahl sichtbarer EintrÑge */
						0,				/* erster sichtbarer Eintrag */
						ctrl_objs1, objs1, LBOX_VERT + LBOX_REAL + LBOX_SNGL + LBOX_TOGGLE, 20,	/* Scrollverzîgerung */
						NULL,			/* user data */
						NULL, 0, 0, 0, 0);
	if (!sbox1)
		Pterm((int) ENSMEM);

	sbox2 = lbox_create(adr_ica_dialog, select_item2, set_item2, sc,	/* Items */
						5,				/* Anzahl sichtbarer EintrÑge */
						0,				/* erster sichtbarer Eintrag */
						ctrl_objs2, objs2, LBOX_VERT + LBOX_REAL + LBOX_SNGL + LBOX_TOGGLE, 20,	/* Scrollverzîgerung */
						NULL,			/* user data */
						NULL, 0, 0, 0, 0);
	if (!sbox2)
		Pterm((int) ENSMEM);
...
int delete_dat(int dat)
{
	...
	for (i = 0; i < linn; i++)
	{
		if (linx[i].datnr == dat)
		{
			if ((linx[i].reldatnr == 0) && ((i + 1 >= linn) || (linx[i + 1].pgmnr != pgm)))
			{
  ...
  no_del:
	...
	for (prev = &(pgmx[pgm].types); *prev >= 0; prev = &datx[*prev].next)
	{
		if (*prev == dat)
		{
...
int insert_pgm(struct pgm_file *pgm)
{
  ...
  redraw:
	...
	if (selected_program)				/* Zeile selektiert, dann... */
	{
		...
		j = (int) (selected_pgmline - linx);	/* ...nicht scrollen! */
	}
	setup_programs(j);
	if (!selected_program)
	{
...
static void chg_icon(int win, int meinding, int iconnr)
{
	...
	for (i = 0; i < NLINES; i++)
	{
		if (win == PGMS)
			...
			/*  if  (win == DFLS)   */
			ding = (i + n < selected_program->ntypes) ? sel_dfiles[i + n].datnr : -1;

		if (ding != meinding)
			continue;
		j = (win == PGMS) ? showindex_to_pgmicnobj(i) : showindex_to_daticnobj(i);
...
static void deselect_program(DIALOG * d)
{
	...
	if (selected_datfile)
	{
		...
		if (selected_datline)
		{
			selected_datline->selected = FALSE;
			selected_datline = NULL;
		}
		if (selected_spdfile)
		{
...
WORD cdecl hdl_ica(struct HNDL_OBJ_args args)
{
	struct zeile *oldsel;
	...
	if (args.obj == HNDL_INIT)
	{
		...
		{
			cmd_pgm_path = ((char **) args.data)[0];
			cmd_dat_path = ((char **) args.data)[1];
			...
			if (args.clicks == 0)		/* Icon fÅr Programm */
			{
				if (cmd_pgm == -1)		/* neues Programm */
					retcode = 2;		/* ... erst anmelden */
			} else if (args.clicks == 1)	/* Icon fÅr Datei */
			{
				if (cmd_dat == -1)		/* neue Datei */
					retcode = 2;		/* ... erst anmelden */
			} else if (args.clicks == 2)	/* Anw. anmelden */
			{
				if (cmd_pgm != -2)
	...
	pgm = pgmobj_to_showindex(args.obj);
	if ((args.clicks == 2) && (pgm >= 0))
	{
		pgm += lbox_get_first(sbox1);
		pgm = linx[pgm].pgmnr;
	...
	dat = datobj_to_showindex(args.obj);
	if ((args.clicks == 2) && (dat >= 0))
	{
		dat += lbox_get_first(sbox2);
		if (selected_program)
			dat = sel_dfiles[dat].datnr;
		...
		(args.obj == PRG1) || (args.obj == PRG2) || (args.obj == PRG3) || (args.obj == PRG4) || (args.obj == PRG5))
	{
		...
		} else if (selected_pgmline && !oldsel)
		{
		...
		(args.obj == DAT1) || (args.obj == DAT2) || (args.obj == DAT3) || (args.obj == DAT4) || (args.obj == DAT5))
	{
		...
		if (selected_program)
		{
			...
			} else if (selected_spdfile != oldsel2)
			{
				dat_was_selected(args.dialog, selected_spdfile->datnr);
			}
		} else
		{
			...
			{
				deselect_dfile(args.dialog);
			} else if (selected_datline != oldsel)
			{
	...
	if (args.obj == DEL_PGM)
	{
		...
		if (selected_pgmline)
		{
			selected_pgmline->selected = FALSE;
			selected_pgmline = NULL;
		}
		delete_pgm(sel);
		goto ende;
