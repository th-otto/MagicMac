...
WORD simple_popup(OBJECT *tree, WORD obj_index, const char *const *names, WORD num_names, WORD selected)
{
	...
	newtree = (OBJECT *)Malloc((num_names + num_names + 1) * sizeof(OBJECT));
	if (newtree != NULL)
	{
		OBJECT *o;
		WORD y;
		...
		mt_objc_offset(tree, obj_index, &newtree[ROOT].ob_x, &newtree[ROOT].ob_y, NULL);
		newtree[ROOT].ob_y -= sel_y;
		selected = mt_form_popup(newtree, 0, 0, NULL);
		if (selected > 0)
		{
			selected -= 1;
			selected /= 2;
