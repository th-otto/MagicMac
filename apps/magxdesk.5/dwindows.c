...
static void sort_mydtas(WINDOW *w)
{
	...
	{
		mdta = *dtas++;
		match = ((mdta->attrib & FA_SUBDIR) || (pattern_match(w->maske, mdta->filename)));
		if (match)
		{
...
static void set_wind_icons(WINDOW *mywindow)
{
	...
	for (z = mywindow->pmydta; z < mywindow->pmydta + anzahl; z++)
	{
		ziele = *z;
		ziele->flags = 0;				/* Datei */
...
static long read_wind(WINDOW *w, int free_flag)
{
	...
	do
	{
		...
		if ((w->real_drive != real_drv) || (real_drv == 'U' - 'A'))
			free_flag = TRUE;
...
/*			long slsize;	*/
			...
			if ((real_drv == 'U' - 'A') || dinfo[real_drv].b_clsiz == 0L)
...
*/
		...
		maxmem_per_name = (int) (sizeof(MYDTA) + maxnamelen + 1 + sizeof(MYDTA *) + sizeof(OBJECT) + sizeof(CICONBLK));
		if (maxmem_per_name & 1)
			maxmem_per_name++;			/* auf WORD runden */

		err = Fxattr(0, w->path, &xa);
		if (err)
			continue;
		w->mtime = xa.st_mtim.u.d.time;
		w->mdate = xa.st_mtim.u.d.date;
		...
		epath = path + strlen(path);	/* hier Dateiname ansetzen */
		ziele = w->memblk;
		limit = ziele + w->memblksize - sizeof(OBJECT) - maxmem_per_name;	/* Objekt #0 berÅcksichtigen! */
		...
		while (err == E_OK)
		{
			...
			if (ziele >= limit)
			{
				...
				if ((w->memblksize >= LASTMAXMEMBLK) || (w->memblksize < FIRSTMAXMEMBLK))
					break;

				/* Versuche Blockvergrîûerung */
				/* -------------------------- */

				if (Mshrink(w->memblk, w->memblksize + NEXTMEMBLK))
				{
					char *newblk;
			...
			if ((!status.show_all) && (xa.st_attr & (FA_HIDDEN | FA_SYSTEM)))
			...
			if (err_xr == E_OK)
			{
				if ((xa.st_mode & S_IFMT) == S_IFLNK)
				{
					...
					if (!err)
						...
						mdta->is_alias = 2;	/* nicht aufgelîst!! */
					err = E_OK;
				}
	...
	for (p_d = w->pmydta, ziele = w->memblk; anzahl > 0; anzahl--)
	{
		mdta = (MYDTA *) ziele;
		*p_d++ = mdta;
		ziele += sizeof(MYDTA) + strlen(mdta->filename) + 1;
	...
	if (status.showtyp == M_ATEXT)
	{
		...
		if (status.font_is_prop)
		{
			...
			for (i = 1; i <= anzahl; i++, p_d++)
			{
				d = *p_d;
#pragma warn -pia
				if (status.show_8p3 && w->dos_mode && (s = strchr(d->filename, '.')))
#pragma warn .pia
				{
					*s = EOS;
		...
		{
			len = (w->dos_mode && status.show_8p3) ? 12 : w->max_fname;
			w->showw = char_w * (4 + len + 8 * status.is_groesse + 10 * status.is_datum + 7 * status.is_zeit);
		}
	} else
	{
	...
	for (i = 1; i <= anzahl; i++, c++, p_d++)
	{
		...
		if (status.showtyp == M_ABILDR)
		{
			...
			o->ob_state = ((!dsel_flag) && (d->flags & 2)) ? SELECTED + WHITEBAK : NORMAL + WHITEBAK;
			if (d->is_alias)
		...
		} else
		{
			...
			o->ob_state = ((!dsel_flag) && (d->flags & 2)) ? SELECTED : NORMAL;
		}
		o->ob_width = w->showw;
		o->ob_height = w->showh;
	}
...
static int re_arrange(WINDOW *w)
{
	...
	if (anzahl < cols)
		cols = anzahl;
	return (oldcols != cols || oldshift != w->yscroll || old_xscroll != w->xscroll);
}
...
WINDOW *create_wnd(int wnr, char *path, char *mask, int yscroll, int flags, LONG *errcode)
{
	WINDOW *w;
	int iconify_flags;
	...
	if (w)
	{
		...
		{
			if (w->handle >= 0)
				wind_set_int(w->handle, WF_BEVENT, BEVENT_WORK + BEVENT_INFO);
			w->yscroll = yscroll;
			w->xscroll = 0;
			w->flags = flags;
...
long opn_wnd(WINDOW *w, int fast)
{
	...
	if (w->flags & WFLAG_ICONIFIED)
	{
		...
		oldg = w->out;
		MY_iconified(w, NULL, (w->flags & WFLAG_ALLICONIFIED) ? ICONIFIED_MODE_ALL : ICONIFIED_MODE_NORMAL);
		wind_set_grect(w->handle, WF_UNICONIFYXYWH, &oldg);
	} else
	{
		dinfo[drv].b_clsiz = 0L;		/* ungÅltig machen */
		...
		if (!fast && (i >= 0))
			graf_growbox_grect((GRECT *) & ((fenster[0]->pobj + i + 1)->ob_x), &w->out);
...
static void MY_closed(WINDOW *w, int kstate)
{
	...
	if (i < n_deskicons)
		graf_shrinkbox_grect((GRECT *) (&(fenster[0]->pobj + i + 1)->ob_x), &w->out);
}
...
static void MY_iconified(WINDOW *w, GRECT *g, int mode)
{
	...
	{
		Mfree(w->memblk);
		w->memblksize = sizeof(MYDTA) + 128 + sizeof(MYDTA *) + 2 * sizeof(OBJECT);
...
static void upd_disk(int lw, int free_flag)
{
	...
	{
		w = *pw;
		if ((!w) || (w->flags & WFLAG_ICONIFIED) || (w->real_drive != lw))
			continue;
...
/****************************************************************
*
* Sucht einen Dateinamen im Fenster und gibt ggf. seine
* interne Nummer 0..realnum-1 zurÅck.
* RÅckgabe -1, wenn nicht gefunden.
*
****************************************************************/

static int find_filename(WINDOW *w, char *fname)
{
	MYDTA **p;
	int i;


	for (i = 0, p = w->pmydta; i < w->realnum; i++, p++)
	{
		if (!strcmp((*p)->filename, fname))
			return (i);
	}
	return (-1);
}
...
/****************************************************************
*
* Geht die Fensterliste durch und macht ggf. einen
* Auto-Update
*
****************************************************************/

void dirwindow_autoupdate(void)
{
	int wnr;
	int j;
	WINDOW **pw;
	...
	MYDTA **pf;
	int num;
	static int is_MgMc = -1;



	if (is_MgMc < 0)
		is_MgMc = (getcookie('MgMc') != NULL);

	for (wnr = 1, pw = fenster + 1; wnr <= ANZFENSTER; wnr++, pw++)
	{
		w = *pw;
		if ((!w) || (w->flags & WFLAG_ICONIFIED))
			continue;
		if ((!is_MgMc) && (w->real_drive < 2))
			continue;					/* Auf Atari und PC nicht fÅr A: B: */

		err = Fxattr(0, w->path, &xa);
		if ((!err) && ((xa.st_mtim.u.d.time != w->mtime) || (xa.st_mtim.u.d.date != w->mdate)))
		{

			/* Alten Fensterinhalt retten */
			/* -------------------------- */

			strcpy(old_sel, w->sel_maske);
			old_pmydta = w->pmydta;
			old_pobj = w->pobj;
			old_memblk = w->memblk;
			old_realnum = w->realnum;
#if 0
			old_size = w->memblksize;
#endif
			w->memblk = NULL;
			...
			} else
			{
				...
				if ((old_realnum == w->realnum) && (old_size == w->memblksize))
				{
					if (old_realnum)
					{
						s1o = (char *) old_pmydta[0];
						f = old_pmydta[old_realnum - 1];
						s2 = (char *) f;
						s2 += strlen(f->filename);
						old_size = s2 - s1o;

						s1n = (char *) w->pmydta[0];
						f = w->pmydta[old_realnum - 1];
						s2 = (char *) f;
						s2 += strlen(f->filename);
						size = s2 - s1n;
					} else
					...
					/* Fensterinhalt unverÑndert */
					/* ------------------------- */

					if ((size == old_size) && (!memcmp(s1o, s1n, size)))
					{
						w->pobj = old_pobj;
						w->pmydta = old_pmydta;
						Mfree(w->memblk);
				...
				/* Select-Status kopieren */
				/* ---------------------- */

				if (old_sel[0])
				{
					strcpy(w->sel_maske, old_sel);
					pf = w->pmydta;
					for (j = 0; j < w->realnum; j++)
					{
						f = *pf++;

						/* paût die Maske ? */

						if (pattern_match(w->sel_maske, f->filename))
							f->flags |= 2;
					}
				} else
				{
					old_pobj++;			/* Hintergrund weglassen */
					for (j = 0; j < old_realnum; j++, old_pmydta++, old_pobj++)
					{
						if (!(old_pobj->ob_state & SELECTED))
							continue;
						num = find_filename(w, (*old_pmydta)->filename);
						if (num >= 0)
						{
							w->pmydta[num]->flags |= 2;
						}
					}
				}
		...
		if (err)						/* Fehler => Fenster schlieûen */
		{
			w->closed(w, 0);
			continue;
		}
...
void auto_close_windows(void)
{
	...
	for (i = 1, pw = fenster + 1; i <= ANZFENSTER; i++, pw++)
	{
		...
		if ((w) && (w->path[0]))
		{
			...
			if ((ret == EDRIVE) || (ret == 0L) || (ret == ELOCKED))
			{
...
void upd_selmask(WINDOW *w)
{
	...
	if (g.g_w && g.g_h)
	{
		...
		if (first_sel_line >= 0)
		{
			if ((first_sel_line < w->yscroll) || (first_sel_line >= w->yscroll + w->wlins))
			{
...
void upd_infos(void)
{
	...
	{
		w = *pw;
		if ((w) && dirty[i] && !(w->flags & WFLAG_ICONIFIED))
		{
			set_info(w);
			wind_set_str(w->handle, WF_INFO, w->info);
			dirty[i] = FALSE;
...
void upd_col(void)
{
	...
	{
		w = *pw;
		if ((w) && (w->shownum > 1) && !(w->flags & WFLAG_ICONIFIED))
		{
			if (status.is_1col)
			{
				if (w->cols > 1)
...
static void optimalsize_wnd(WINDOW *w)
{
	...
	g.g_x = w->in.g_x;
	g.g_y = w->in.g_y;
	g.g_w = w->xoffs + cols * (w->showw + w->xdist) - w->xdist;
  ...
  ok:
	if ((w->out.g_x == og.g_x) && (w->out.g_y == og.g_y) && (w->out.g_w == og.g_w) && (w->out.g_h == og.g_h))
...
static void MY_keyed(WINDOW *w, int kstate, int key)
{
	...
	if (!(kstate & K_ALT) && shift && (kstate & (K_CTRL)))
	{
...
/* SH-Cursor up */
	case 0x4838:
		w->arrowed(w, WA_UPPAGE);
		break;
/* SH-Cursor dwn */
	case 0x5032:
		w->arrowed(w, WA_DNPAGE);
		break;
/* Esc */
	case 0x011b:
		if (shift)
