...
int starticn[0x002c] = { 0x0030, 0x0000, 0x0078, 0x0000,
...
int smallicn[0x0010] = { 0x0000, 0x0100, 0x0380, 0x07C0,
...
OBJECT bigbutton[] = {
	...
	{0, -1, -1, G_BUTTON, LASTOB | SELECTABLE | DEFAULT | EXIT | FL3DACT, DISABLED, (long) "Start", 0x0403, 0x0400,
	 0x0008, 0x0001}
};
...
OBJECT smallbutton[] = {
	...
	{0, -1, -1, G_BOXTEXT, LASTOB | SELECTABLE | DEFAULT | EXIT | FL3DACT, DISABLED, (long) &rs_tedinfo, 0x0102, 0x0100,
	 0x0406, 0x0e00}
};
...
char *helpbuf = NULL,
	...
	*bubblehelp = NULL,
	...
	bubbleinfo[] =
	"Wenn Sie diesen Bereich des SMU-Buttons mit der linken Maustaste anklicken und diese gedrckt halten, k”nnen Sie den SMU-Button auf dem Desktop verschieben.",
/*            bubbleinfo2[] = "Verschiebt den Button bei gedrckter Maustaste", */
	...
	bubbleinfo[] =
	"Hvis du klikker her med venstre musknapp og holder knappen nede, kan du flytte SMU-knappen rundt om p† desktopen.",
#else
#ifdef SWEDISH
	drventry[] = "  Enheter",
	docentry[] = "  Dokument",
	...
	xaccusr[] = "1Win95 Start knapp",
	bubbleinfo[] =
	"Om du trycker med v„sterknappen i denna area av SMU, och h†ller mus-knappen nedtryckt, s† kan du flytta SMU till n†gon annan plats p† skrivbordet.",
#else
	drventry[] = "  Drives",
	docentry[] = "  Documents",
	appentry[] = "  Applications",
	...
	bubbleinfo[] =
	"If you click in this area of the SMU button with the left mouse button and keep the mouse button pressed you can move the SMU button on the desktop.",
#endif
#endif
#endif
/*            startpath[128], */
...
int remove_cookie(long id)
{
	...
	if (cookiejar)
	{
		while ((cookiejar->id) && (cookiejar->id != id))
			cookiejar++;
...
int appl_xgetinfo(int type, int *out1, int *out2, int *out3, int *out4)
{
	...
	has_agi = ((_GemParBlk.global[0] == 0x399 && get_cookie('MagX', NULL))
			   ...
			   ||(appl_find("?AGI") >= 0)) || (wind_get(0, WF_WINX, &dummy, &dummy, &dummy, &dummy) == WF_WINX);
...
long call_hsn(void)
{
	if (cs->load_sound(cs_fname))
		cs->play_it(&cs->lbuf, 1, 0);
	return (0);
}
...
void main(int argc, const char *argv[])
{
	...
	if (ap_id >= 0)
	{
		...
		if (!get_system_parameter((argc == 2) ? (char *) argv[1] : NULL))
			goto _raus;
		...
		if (get_cookie('MagX', (long *) &cv))
		{
			if (cv->aesvars)
			{
				...
				mxdate =
					(cv->aesvars->date << 16) | (cv->aesvars->date >> 24) | ((cv->aesvars->date >> 8) & 0x0000ff00L);
		...
		open_main_window();
		if (whandle > 0)
			wind_set(whandle, WF_BOTTOM, 0, 0, 0, 0);
...
void create_links(void)
{
	...
	if (ret >= 0L)
	{
		...
		while (Fread(handle, sizeof(LINK), &link) == sizeof(LINK))
			Fsymlink(link.path, link.name);
		...
		if ((handle & 0xff000000L) != 0xff000000L)
		{
			while (!Dreaddir(36, handle, name))
			{
				...
				if (!Fxattr(1, link.name, &attr))
				{
					...
					{
						if (!Freadlink(256, link.path, link.name))
							Fwrite(fhdl, sizeof(LINK), &link);
					}
				}
			}
...
void final_init(void)
{
	...
	if (desktop)
	{
		...
		wind_set(DESK, WF_NEWDESK, (int) (((long) &startdesk >> 16) & 0x0000ffffL),
				 (int) ((long) &startdesk & 0x0000ffffL), ROOT, 0);
...
int find_avserver(int avprot)
{
	...
	if (avserver >= 0)
	{
		if (avserver == appl_find(servername))
			return (1);
	}
	...
	if (dummy)
	{
		...
		servername[8] = 0;
		while (strlen(servername) < 8)
			strcat(servername, " ");
		for (i = 0; i < 8; i++)
			servername[i] = toupper(servername[i]);
				...
				while (dummy)
				{
					if (strlen(dummy))
					{
						...
						case 0:
							if (dummy[0] == '#')
								goto _nextline;
							else if (!stricmp(dummy, "/file"))
								state = 10;
							else if (!stricmp(dummy, "/menu"))
								state = 20;
							...
							} else if (!stricmp(dummy, "/separator"))
							{
								...
								goto _nextline;
							} else if (!stricmp(dummy, "/group"))
								state = 25;
							else if (!stricmp(dummy, "/folder"))
								state = 50;
							else if (!stricmp(dummy, "/alias"))
								state = 100;
							else if (!stricmp(dummy, "/timer"))
								state = 110;
							else if (!stricmp(dummy, "/label"))
								state = 120;
							else if (!stricmp(dummy, "/shutdown"))
								state = 30;
							else if (!stricmp(dummy, "/shutdown_timeout"))
								state = 32;
							else if (!stricmp(dummy, "/documents_max"))
								state = 33;
							else if (!stricmp(dummy, "/documents_maxperapp"))
								state = 34;
							else if (!stricmp(dummy, "/logfile"))
								state = 35;
							else if (!stricmp(dummy, "/applications_max"))
								state = 36;
...
/*									applications=1; */
							...
							} else if (!stricmp(dummy, "/backwind"))
							{
								untop = 1;
								goto _nextline;
							} else if (!stricmp(dummy, "/resource"))
								state = 70;
						...
						case 40:
							{
								...
								if (fle)
								{
									...
									if (!stricmp(smp, "boot"))
										sampleboot = fle;
									else if (!stricmp(smp, "open"))
										sampleopen = fle;
									else if (!stricmp(smp, "start"))
										samplestart = fle;
									else if (!stricmp(smp, "avfail"))
										sampleavfail = fle;
									else if (!stricmp(smp, "term"))
										sampleterm = fle;
									else if (!stricmp(smp, "close"))
										sampleclose = fle;
									else if (!stricmp(smp, "timer"))
										sampletimer = fle;
								}
							}
							goto _nextline;
...
OBJECT *build_popup(STARTMENU *curr, int drv, int docs, int apps)
{
	...
	while (ce)
	{
		...
		if (ce->name)
		{
			if ((int) strlen(ce->name) > maxw)
				maxw = (int) strlen(ce->name);

			if (stic)
			{
				if (ce->file)
				{
					if (stic->str_icon(ce->file, STIC_SMALL))
						sticons++;
				}
			}
		}
		...
		if (pop)
		{
			...
			if (drv)
			{
				...
				if (pop[c].ob_spec.free_string)
				{
					...
					while (strlen(pop[c].ob_spec.free_string) < maxw)
						strcat(pop[c].ob_spec.free_string, " ");
				}
			}
			...
			while (ce)
			{
				...
				if (!pop[c].ob_spec.free_string)
					pop[c].ob_spec.free_string = ce->name;
				else
				{
					if (ce->name)
					{
...
int add_dhst(DHSTINFO *dhstinfo)
{
	...
	if (newdhst->info.appname)
		strcpy(newdhst->info.appname, dhstinfo->appname);

	newdhst->info.apppath = (char *) malloc(strlen(dhstinfo->apppath) + 1L);
	if (newdhst->info.apppath)
		strcpy(newdhst->info.apppath, dhstinfo->apppath);

	newdhst->info.docname = (char *) malloc(strlen(dhstinfo->docname) + 1L);
	if (newdhst->info.docname)
		strcpy(newdhst->info.docname, dhstinfo->docname);
...
void load_dhst(void)
{
	...
	if (ret >= 0L)
	{
		...
		while (danz)
		{
			Fread(handle, 4, &slen);
			if (slen)
				Fread(handle, slen, dinfo.appname);
			else
			...
			Fread(handle, 4, &slen);
			if (slen)
				Fread(handle, slen, dinfo.docname);
			else
				strcpy(dinfo.docname, "");

			Fread(handle, 4, &slen);
			if (slen)
				Fread(handle, slen, dinfo.docpath);
			else
...
void save_inf(int savedhst)
{
	...
	if (startinf.workxabs <= desk.g_x)
		startinf.workxabs = -2000;
	if (startinf.workxabs + startbutton[ROOT].ob_width >= desk.g_x + desk.g_w)
		startinf.workxabs = 30000;

	if (startinf.workyabs <= desk.g_y)
		startinf.workyabs = -2000;
	if (startinf.workyabs + startbutton[ROOT].ob_height >= desk.g_y + desk.g_h)
		startinf.workyabs = 30000;
...
{
	if (startinf.workxabs + startbutton[ROOT].ob_width > desk.g_x + desk.g_w)
		startinf.workxabs = desk.g_x + desk.g_w - startbutton[ROOT].ob_width;
	if (startinf.workxabs < desk.g_x)
		startinf.workxabs = desk.g_x;

	if (startinf.workyabs + startbutton[ROOT].ob_height > desk.g_y + desk.g_h)
		startinf.workyabs = desk.g_y + desk.g_h - startbutton[ROOT].ob_height;
	if (startinf.workyabs < desk.g_y)
...
void open_main_window(void)
{
	...
	if (whandle <= 0)
	{
		...
		wind_calc(WC_BORDER, WINDOWSTYLE, startbutton[ROOT].ob_x, startbutton[ROOT].ob_y, startbutton[ROOT].ob_width,
				  startbutton[ROOT].ob_height, &x, &y, &w, &h);
...
void start_app(int force, int how, char *file, char *cmd)
{
	...
	{
		if ((!stricmp(dummy, ".ACC")) || (!stricmp(dummy, ".ACX")))
			doex = SHW_LOAD_ACC;
		else
		{
			if ((!stricmp(dummy, ".TOS")) || (!stricmp(dummy, ".TTP")))
				gem = 0;
		}

		*dummy = 0;

		if ((!gem) && ((avnotos) || (!find_avserver(1))))
			goto _vastart;
	}
...
void start_popup(char *dcmd, int dx, int dy, int dflags, int what, char *mname)
{
	...
	if (startbutton[btnidx].ob_state & DISABLED)
		return;
	...
	if (drives)
	{
		...
		for (c = 0, b = 1; c < 32; c++)
		{
			if (drvmap & b)
			{
				...
				if (labels[c])
				{
					if (strlen(labels[c]) > lmax)
						lmax = strlen(labels[c]);
		...
		if (drvpop)
		{
			...
			if (stic)
				stic->menu_attach(ME_ATTACH, menus->tree, 1, &menu);
			else
			...
			if (docpop)
			{
				...
				while (d)
				{
					...
					if (docpop[cnt].ob_spec.free_string)
					{
						...
						if (d->info.appname)
							strcat(docpop[cnt].ob_spec.free_string, d->info.appname);
						strcat(docpop[cnt].ob_spec.free_string, "] ");
						if (d->info.docname)
							strcat(docpop[cnt].ob_spec.free_string, d->info.docname);

						while (strlen(docpop[cnt].ob_spec.free_string) < lmax - 1)
							strcat(docpop[cnt].ob_spec.free_string, " ");
					}
				...
				if (stic)
					stic->menu_attach(ME_ATTACH, menus->tree, docidx, &menu);
				else
	...
	{
	case WHAT_HISTORY:
		if (mnx + docpop[ROOT].ob_width > desk.g_x + desk.g_w)
			mnx = desk.g_x + desk.g_w - docpop[ROOT].ob_width;
		break;
	case WHAT_DRIVES:
		if (mnx + drvpop[ROOT].ob_width > desk.g_x + desk.g_w)
			mnx = desk.g_x + desk.g_w - drvpop[ROOT].ob_width;
		break;
	case WHAT_MENU:
		if (mnx + whatmenu[ROOT].ob_width > desk.g_x + desk.g_w)
			mnx = desk.g_x + desk.g_w - whatmenu[ROOT].ob_width;
		break;
	default:
		if (mnx + menus->tree[ROOT].ob_width > desk.g_x + desk.g_w)
			mnx = desk.g_x + desk.g_w - menus->tree[ROOT].ob_width;
	}
	...
	if (ret)
	{
		...
		if (start)
		{
			...
			if (start == menus)
				msel.mn_item -= (drives + documents + applications);
	...
	if (playclose)
		play_sound(sampleclose);

  _drvend:
	if (drvpop)
	{
		if (stic)
			stic->menu_attach(ME_REMOVE, menus->tree, 1, NULL);
		else
	...
	{
		if (stic)
			stic->menu_attach(ME_REMOVE, menus->tree, 1 + drives, NULL);
		else
...
void init_shutdown(void)
{
	if (strlen(shutdown))
	{
		long ret = Fopen(shutdown, FO_READ),
			len = 0;
		...
		if (len > 20000L)
			start_app(0, SHW_PARALLEL, shutdown, NULL);
		else
		{
			cleanup(0);
			apterm = 1;
...
void info_popup(int mx, int my)
{
	...
	if (mnx + startpopup[ROOT].ob_width > desk.g_x + desk.g_w)
		mnx = desk.g_x + desk.g_w - startpopup[ROOT].ob_width;
	...
	findstic();
	if (stic)
		ret = stic->menu_popup(&menu, mnx, my, &msel);
	else
	...
	if (ret)
	{
		...
		case 5:
			...
			{
				if (form_alert(1, message[MSG_QUIT]) == 1)
					quit = 1;
			}
			break;
...
{
	if (!bubblehelp)
		bubblehelp = (char *) Mxalloc(256L, ALLOCMODE);
...
void handle_button(int mx, int my, int bstate, int kstate, int clicks)
{
	...
	if ((!between(mx, x, x + w - 1)) || (!between(my, y, y + h - 1)))
		return;
	...
	if (bstate == 1)
	{
		if (kstate & KsSHIFT)
			move_button();
		else if (objc_find(startbutton, ROOT, MAX_DEPTH, mx, my) == btnidx)
		{
		...
		if (rc_intersect(&dirty, &work))
		{
			...
			while ((box.g_w > 0) && (box.g_h > 0))
			{
				if (rc_intersect(&work, &box))
					objc_draw(startbutton, ROOT, MAX_DEPTH, box.g_x, box.g_y, box.g_w, box.g_h);
...
int doGSCommand(int pipe[8])
{
	...
	if (cmd)
	{
		...
		} else if (!stricmp(cmd, "OpenMenu"))
		{
			...
			if (cmd)
				open_over_mouse(NULL, WHAT_MENU, cmd);
			else
				open_over_mouse(NULL, WHAT_NORMAL, NULL);
...
int handle_message(int pipe[8])
{
	...
	case MN_SELECTED:
		...
		if (pipe[3] >= 0)
			menu_tnormal(deskmenu, pipe[3], 1);
		break;

	case WM_REDRAW:
		if (pipe[3] == whandle)
			redraw(pipe[4], pipe[5], pipe[6], pipe[7]);
		break;

	case WM_ONTOP:
	case WM_NEWTOP:
		if ((pipe[3] == whandle) && (untop) && (appline < 0))
			wind_set(whandle, WF_BOTTOM, 0, 0, 0, 0);
		break;
	...
	case WM_MOVED:
		if (pipe[3] == whandle)
		{
			...
			if (between(x, desk.g_x, desk.g_x + desk.g_w - 1) && between(y, desk.g_y, desk.g_y + desk.g_h - 1)
				|| (magxfadeout))
			{
				startinf.workxabs = x;
				startinf.workyabs = y;
				...
				wind_calc(WC_BORDER, WINDOWSTYLE, startbutton[ROOT].ob_x, startbutton[ROOT].ob_y,
						  startbutton[ROOT].ob_width, startbutton[ROOT].ob_height, &x, &y, &w, &h);
	...
	case AP_DRAGDROP:
		{
			...
			if (res >= 0L)
			{
				...
				if (Fwrite(handle, 1, &c) != 1)
					goto _perror;
				if (Fwrite(handle, 32, ptypes) != 32)
					goto _perror;
				if (Fread(handle, 2, &hlen) != 2)
					goto _perror;
				if (Fread(handle, 4, &dtype) != 4)
					goto _perror;
				if (Fread(handle, 4, &dsize) != 4)
					goto _perror;
				...
				while (hlen > 0)
				{
					if (Fread(handle, min(hlen, 32), ptypes) != min(hlen, 32))
						goto _perror;
					hlen -= 32;
				}
				...
				if (dtype == 'ARGS')
				{
					...
					if (Fread(handle, dsize, pcmd + 1) != dsize)
						goto _perror;
					success = 1;
				}
	...
	case BUBBLEGEM_REQUEST:
		if (objc_find(startbutton, ROOT, MAX_DEPTH, pipe[4], pipe[5]) != btnidx)
			show_bubblehelp(pipe[4], pipe[5]);
		break;
	...
	case GS_REQUEST:
		{
			...
			if (!gsi)
				gsi = (GS_INFO *) Mxalloc(sizeof(GS_INFO), ALLOCMODE);
...
void start_timer(TIMER *dtimer)
{
	if (strlen(logfile))
	{
		...
		if (ret >= 0L)
		{
			...
			handle = (int) ret;
			if (Fseek(0, handle, 2) < 0L)
				goto _error;

			if (print_two(handle, d & 0x001f))
				goto _error;
			if (Fwrite(handle, 1L, ".") != 1L)
				goto _error;
			if (print_two(handle, (d >> 5) & 0x000f))
				goto _error;
			if (Fwrite(handle, 1L, ".") != 1L)
				goto _error;
			itoa(((d >> 9) & 0x007f) + 1980, num, 10);
			if (Fwrite(handle, strlen(num), num) != strlen(num))
				goto _error;
			if (Fwrite(handle, 1L, " ") != 1L)
				goto _error;

			if (print_two(handle, (t >> 11) & 0x001f))
				goto _error;
			if (Fwrite(handle, 1L, ":") != 1L)
				goto _error;
			if (print_two(handle, (t >> 5) & 0x003f))
				goto _error;
			if (Fwrite(handle, 1L, ":") != 1L)
				goto _error;
			if (print_two(handle, (t & 0x001f) << 1))
				goto _error;
			if (Fwrite(handle, 2L, "\x0d\x0a") != 2L)
				goto _error;

			if (print_two(handle, (int) ((dtimer->time >> 11) & 0x0000001f)))
				goto _error;
			if (Fwrite(handle, 1L, ":") != 1L)
				goto _error;
			if (print_two(handle, (int) ((dtimer->time >> 5) & 0x0000003f)))
				goto _error;
			if (Fwrite(handle, 2L, "  ") != 2L)
				goto _error;

			if (Fwrite(handle, strlen(dtimer->file), dtimer->file) != strlen(dtimer->file))
				goto _error;
			...
			if (dtimer->fcmd)
			{
				if (Fwrite(handle, 2L, " (") != 2L)
					goto _error;
				if (Fwrite(handle, strlen(dtimer->fcmd + 1L), dtimer->fcmd + 1L) != strlen(dtimer->fcmd + 1L))
					goto _error;
				if (Fwrite(handle, 1L, ")") != 1L)
					goto _error;
			}
