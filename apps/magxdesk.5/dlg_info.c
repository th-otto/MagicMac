...
static WORD _CDECL hdl_info_file( struct HNDL_OBJ_args args );
...
static _WORD _CDECL hdl_info_file(struct HNDL_OBJ_args args)
{
	...
	if	( args.obj < 0 )					/* Nachricht? */
		...
		if	(args.obj == HNDL_INIT)
			{
			ifd = (INFO_FILE_DATA *) wdlg_get_udata( args.dialog );
			ifd->dialog = args.dialog;
		...
		{
		ifd = (INFO_FILE_DATA *) args.data;
		tree = ifd->tree;

		if	( args.obj == FI_SIZER )		/* Grîûe Ñndern */
			{
			graf_rubbox(
						tree->ob_x, tree->ob_y,
		...
		if	( args.obj == FI_EXP )
			...
			if	(ifd->extended_view)
				...
				subobj_wdraw(args.dialog, FI_OK, ROOT, MAX_DEPTH);
				subobj_wdraw(args.dialog, FI_CAN, ROOT, MAX_DEPTH);
				subobj_wdraw(args.dialog, FI_SIZER, ROOT, MAX_DEPTH);
				subobj_wdraw(args.dialog, FI_XDRIVEATTR, ROOT, MAX_DEPTH);
			...
			subobj_wdraw(args.dialog, args.obj, 0, MAX_DEPTH);
			return( 1 );
			}

		if	( args.obj == FI_FINDORIGINAL )
			{
			char path[128];
			char *name;
			char c;
			...
			*name = c;
			...
			ob_dsel(tree, args.obj);
			subobj_wdraw(args.dialog, args.obj, 0, 1);
			return( 1 );
			}
		if	( args.obj == FI_CAN )		/* Abbruch */
			{
			return( 0 );
			}
		if	( args.obj == FI_OK )		/* OK */
			{
			exit_info_file_tree( ifd );
			return( 0 );
			}
