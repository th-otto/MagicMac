...
static long test_readonly(char *path, char *fname, XATTR * xa)
{
	int ret;
	...
	if (!((xa->st_attr) & FA_READONLY))
		return 1;					/* OK */
	ret = Rxform_alert(3, ALRT_FILE_RDONLY, path[0], fname, NULL);
	if (ret == 1)
...
static int dial_datexi(char *path, char *fname, filetype ftype, int is_ren)
{
	int whdl;
	long ret;
	FILEDESCR fd;
	...
	d_dat = wdlg_create(hdl_dat, adr_dat, &fd,	/* user_data */
						0,				/* code */
						NULL,			/* data */
						is_ren);		/* flags */
	if (!d_dat)
	...
	whdl = wdlg_open(d_dat,
		 Rgetstring(is_ren ? STR_RENAME : STR_NAMECONFLICT, NULL), NAME + CLOSER + MOVER, -1, -1, 0, NULL);
...
static long pathinfo(char *path, long *free_clusters, long *cluster_size)
{
	_DISKINFO di;
	_DTA dta;
	long errcode;
...
{
	if (abbruch || (Kbshift(-1) & (K_LSHIFT | K_RSHIFT)) == (K_LSHIFT | K_RSHIFT))
	{
		if (abbruch)
			ackn_cancel();
...
static void set_dirty(long err, char *path, char val)
{
	int drv;
...
static long GFdelete(char *path, char *name)
{
...
static long GDdelete(char *path, char *name)
{
	char all[MAX_PATHLEN + 2];
	long doserr;
...
static long GFcreate(char *path, char *fname, XATTR * file)
{
	XATTR newfile;
	char all[MAX_PATHLEN + 2];
	char oldname[MAX_NAMELEN + 2];
	long doserr;
	unsigned int attr;
	int ret;
...
static long GFsymlink(char *buf, char *path, char *fname, XATTR * xa)
{
	XATTR newfile;
		  ...
		  create_symlink:
			...
			if (doserr == EACCDN)
			{
				...
				if (!doserr)
				{
					...
					{
						if (1 == Rxform_alert(2, ALRT_REPL_W_ALIA, path[0], fname, NULL))
							doserr = E_OK;
					} else
...
static long GFrename(char *path, char *fname, char *new_name, XATTR * xa, char *zpath)
{
	XATTR newfile;
	int old_ftype, new_ftype;
	char alls[MAX_PATHLEN + 2];
	char alld[MAX_PATHLEN + 2];
	long doserr;
	int ret;
	...
	old_ftype = ((xa->st_mode & S_IFMT) == S_IFDIR) ? FOLDER : ORDINARYFILE;
...
*/
			...
			new_ftype = ((newfile.st_mode & S_IFMT) == S_IFDIR) ? FOLDER : ORDINARYFILE;
			...
			if (old_ftype != FOLDER)
			{
				...
				if (query == OVERWRITE)
					...
					doserr = does_exist(alld, xa, &newfile);
...
static long GFcopy_symlink(char *path, char *fname, char *new_name, XATTR * xa, char *dstpath)
{
	char alls[MAX_PATHLEN + 2];
	long err;
	char buf[MAX_PATHLEN + 2];
...
static long GFcopy(char *path, char *fname, char *new_name, XATTR * xa, char *zpath)
{
	char alls[MAX_PATHLEN + 2];
	char  alld[MAX_PATHLEN + 2];
	long doserr;
...
static long GDcreate(char *path, char *name)
{
	char all[MAX_PATHLEN + 2];
	long doserr, doserr2;
	int ret;
...
*/
		...
		if (doserr == EACCDN)
		{
			...
			new_ftype = ((newfile.st_mode & S_IFMT) == S_IFDIR) ? FOLDER : ORDINARYFILE;
			if ((new_ftype == FOLDER) && ((copy_mode == BACKUP) || (copy_mode == OVERWRITE)))
...
static long resolve_unknown_ftypes(char *path, long *flen)
{
	XATTR xa;
	char *lastc;
	long err;
	...
	{									/* unbekanntes Objekt */
		...
		else
		{
			...
			if ((xa.st_mode & S_IFMT) == S_IFLNK)
				*flen = -2;
			else if ((xa.st_mode & S_IFMT) == S_IFDIR)
			{
				*flen = -1;
				*lastc = '\\';
			} else if ((xa.st_mode & S_IFMT) == S_IFREG)
				*flen = xa.st_size;
			else
				*flen = -3;				/* irgendein special file */
		}
	}
	return E_OK;
}
...
long prepare_action(int function,
	...
	long *size_netto_src, long *cl_free_dst,
	long *clsize_dst, int argc, char *argv[], char *dest_path)
{
	...
	*n_dat = *n_ord = *size_used_src = *cl_used_dst = 0L;
	...
	{
		err = pathinfo(dest_path, cl_free_dst, &cluster_size_dst);
	}
...
*/
		...
		{								/* Ordner */
			...
			{
				err = walk_path(path, &dummy, &dummy, &folders, &nfiles, &hfiles, 0L, 0L);
				*n_dat += nfiles + hfiles;
				*n_ord += folders;
			}
		}
...
static long _delete(char *path, int ispath, long flen)
{
	...
	if (ispath)
	{
		doserr = walk_path(path, &dummy, &dummy, &dummy, &dummy, &dummy, 0L, after_delete);
		if (doserr || !path[3])
			return doserr;
		endp[-1] = EOS;
		if (qbreak())
...
static long before_cpmv(char *path, char *fname, XATTR * xa)
{
	...
	if (copy_mode == RENAME)
	{
		ret = dial_datexi(_zielpath, new_name, ((xa->st_mode & S_IFMT) == S_IFLNK) ? ALIAS : ORDINARYFILE, TRUE);
		if (ret == 0)					/* öberspringen */
			return E_OK;
		if (ret < 0)					/* Abbruch */
			return EBREAK;
	...
	if (ispath)
	{
		...
		if (quell[3] && move_flag)		/* nicht root */
		{
			...
			if (!doserr)
			{
				c = *name;
				*name = EOS;
				doserr = GFrename(quell, newname, newname, &xa, _zielpath);
				*name = c;
			}
			if (!doserr)
			{
		...
		if (doserr == ENSAME)
		{
			...
			doserr = walk_path(quell, &dummy, &dummy, &dummy, &dummy, &dummy, before_cpmv, after_cpmv);
			endp[-1] = EOS;				/* letzten '\' entfernen */
		}
...
static long copy_move(int move_flag, int cmode, int argc, char *argv[], char *dest_path)
{
	...
	for (; argc > 0; argc -= 2, argv++)
	{
		...
		if (err)
			...
			err = _copy_move(srcpath, dstpath, move_flag, flen == -1);
		if (err == EBREAK)
			break;
	}
...
static long create_aliases(int cmode, int argc, char *argv[], char *dest_path)
{
	char path[MAX_PATHLEN + 2];
	long err, flen;
...
static long delete(int argc, char *argv[])
{
	char path[MAX_PATHLEN + 2];
	long err, flen;
...
LONG cdecl action_thread(void *_par)
{
	...
	{
	case 'A':
		err = create_aliases(copy_mode, par->argc, par->argv, par->dstpath);
		break;
	case 'C':
		err = copy_move(FALSE, copy_mode, par->argc, par->argv, par->dstpath);
		break;
	case 'M':
		err = copy_move(TRUE, copy_mode, par->argc, par->argv, par->dstpath);
		break;
	case 'D':
