...
static void set_selmask(WINDOW *w, char *sel_mask)
{
	...
	if	(sel_mask)
		{
		wmask = w->sel_maske;
		len = olen = strlen(sel_mask);
		if	(len > MAX_SELMASK)
			len = MAX_SELMASK;
		memcpy(wmask, sel_mask, len);
		if	(olen > len)			/* muûte abschneiden */
			wmask[len-1] = '*';		/*  => dann '*' anhÑngen */
		wmask[len] = EOS;
		upperstring(wmask);
		upd_selmask(w);
		}
		...
		if	(app_knows_vastart)
			...
			*((void **) (msg+3)) = param->command_tail;
			...
			if	(0 >= appl_write(-2, 16, &xmsg))
				...
				/* Wenn keine Kommandozeile Åbergeben wurde und
				   kein Fehler aufgetreten ist, App nach oben! */
				if	(!param->length)
					{
					/* erst alle Fenster ... */
					wind_set(0, WF_TOPALL, dst_apid, 0,0,0);
					/* dann MenÅ und Hintergrund */
					wind_set(-1, WF_TOP, dst_apid, 0,0,0);
					}
				}
			return(TRUE);
			}
