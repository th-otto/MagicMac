...
WORD mt_edit_evnt(OBJECT *tree, WORD obj, WORD whdl,
				EVNT *ev, LONG *errc, WORD *global)
{
	PARMDATA d;
	static WORD	c[] = { 215, 2, 3, 2 };

	d.intin[0] = obj;
	d.intin[1] = whdl;
	d.addrin[0] = tree;
	d.addrin[1] = ev;
	*((LONG *) (d.intout+1)) = 0L;	/* fÅr alte SLB */
	__aes( &d, c, global );
	*errc = *((LONG *) (d.intout+1));
	return(d.intout[0]);
}
					...
					WORD *global )
{
	...
	d.addrin[0] = tree;
	d.intout[4] = FALSE;	/* fÅr alte Bibliotheken */
	__aes( &d, c, global );
	*fontID = d.intout[1];
	*fontH = d.intout[2];
	*fontPix = d.intout[4];
...
void mt_edit_set_font( OBJECT *tree, WORD obj,
					WORD fontID, WORD fontH, WORD fontPix,
					WORD mono, WORD *global )
{
	PARMDATA d;
	static WORD	c[] = { 217, 6, 0, 1 };
