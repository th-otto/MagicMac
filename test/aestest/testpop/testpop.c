...
int main(void)
{
	...
	form_center_grect(dial, &g);
	form_dial_grect(FMD_START, &g, &g);
	objc_draw_grect(dial, ROOT, MAX_DEPTH, &g);
	do
	{
		antw = form_do(dial, -1);
		if (antw == POPUP)
		{
			...
			{
				strcpy(dial[POPUP].ob_spec.tedinfo->te_ptext, pop.mn_tree[pop.mn_item].ob_spec.free_string);
				objc_draw_grect(dial, POPUPS, 1, &g);
			}
		}
	}
	while (antw != ABBRUCH);
	form_dial_grect(FMD_FINISH, &g, &g);
