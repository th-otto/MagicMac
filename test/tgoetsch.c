...
int main(void)
{
	...
	dev = form_alert(0, "[2][Welche Geraetenummer fuer VDI?][1|3|6]");
	if ((dev > 0) && (dev < 4))
	{
		itoa(dev, tail + 1, 10);
	...
	if (shel_write(5, devices[dev], 0, NULL, NULL) == 0)
	{
		...
		{
			sprintf(text,
					"[4][Die Anwendung %s|kann sich nicht beenden.]" "[Schade|Anwendung abbrechen]", ignorantname);
	...
	} else
	{
		...
		{
			evnt_mesag(msgbuf);
		} while (msgbuf[0] != RESCH_COMPLETED);
		...
		if (msgbuf[3] == 0)
		{
			...
			sprintf(text, "[4][Die Anwendung %s|"
					"hat den Reschange abgelehnt|Fehlercode = %d]" "[ OK ]", ignorantname, msgbuf[5]);
			form_alert(1, text);
