...
LONG cdecl psub_my_special1( struct PDLG_HNDL_args args)
{
	...
	offset = args.sub->index_offset;								/* Offset fÅr die Objektindizes */
...
void	menu_selected( WORD title, WORD entry )
{
	...
	switch( entry )
	{
		...
		case	DPAPER:
		{
			...
			menu_ienable( menu_tree, DPRINT, 0 );				/* "Drucken" ist nicht anwÑhlbar */
			open_print_dialog( gprn_settings, fstring_addr[S_NONAME], PDLG_PREFS );	/* Einstelldialog îffnen */
			gprn_is_print_dialog = 0;									/* Einstelldialog wird angezeigt */
			break;
		}
		case	DPRINT:
		{
			menu_ienable( menu_tree, DPAPER, 0 );					/* "Papierformat" ist nicht anwÑhlbar */
			open_print_dialog( gprn_settings, fstring_addr[S_NONAME], PDLG_PRINT );	/* Druckdialog îffnen */
	...
	while ( quit == 0 )
	{
		...
		if ( gprn_dialog )
		{
			...
			if ( pdlg_evnt( gprn_dialog, gprn_settings, &events, &button ) == 0 )	/* Dialog schlieûen? */
			{
				...
				{
					menu_ienable( menu_tree, DPAPER, 1 );			/* "Papierformat" ist wieder anwÑhlbar */

					if ( button == PDLG_OK )							/* "Drucken" angewÑhlt? */
						print_sample( gprn_settings );
				}
				else															/* Einstelldialog */
					menu_ienable( menu_tree, DPRINT, 1 );		/* "Drucken" ist wieder anwÑhlbar */
			}
		}
	}
}
