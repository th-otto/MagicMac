...
void DKDirentry2Xattr(LOGICAL_DEV *ldp, DIRENTRY *de, int drive, XATTR *xap)
{
	...
	xap->attr = de->tosattr;
	xap->mode = de->mode;
	xap->index = de->pri.start;
	xap->dev = drive;
	xap->uid = de->uid;
	xap->gid = de->gid;
	xap->nlink = de->nlink;
	xap->size = de->pri.length;
	xap->blksize = ldp->blocksize;
	xap->nblocks = (de->pri.length + ldp->blocksize - 1) / ldp->blocksize;
	xap->nblocks += (de->ass.length + ldp->blocksize - 1) / ldp->blocksize;
	xap->atime = de->atime;
	xap->ctime = de->ctime;
	xap->mtime = de->mtime;
	xap->adate = de->adate;
