...
#ifndef __PUREC_SIGNAL_H__
#define __PUREC_SIGNAL_H__
...
#include <bits/types/sigatomt.h>
#include <bits/types/sigset_t.h>

/*
 * Note that the definitions here are only maintained
 * for compatibility with the original pcstdlib.
 * They are not ANSI, and not MiNT-compatible,
 * especially Psignal() expects different arguments
 * than provided here both for real signals and for
 * constants like SIG_DFL.
 * If you want to link with pcstdlib, but use the
 * signal definitions from MiNT,
 * #define __USE_MINT_SIGNAL (or include <mint/signal.h>
 * instead  of <signal.h>), and link pcmntsig.lib before
 * pcstdlib.lib.
 */ 
...
/* MiNT name */
typedef void (*__sighandler_t) (int signum);
/* original PureC name */
...
#define signal __purec_signal
#define raise __purec_raise
#endif
