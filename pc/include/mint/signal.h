...
typedef void (*__sighandler_t) (int __signum);
...
extern int		__kill	(__pid_t __pid, int) __THROW;
#ifdef __USE_POSIX
extern int kill	(__pid_t __pid, int) __THROW;
#endif
...
extern int __sigpause __P ((long __sig_or_mask, int __is_sig));
...
#ifdef __USE_BSD
extern __mint_int_or_long sigsetmask  __P((long mask));
extern __mint_int_or_long sigblock    __P((long mask));
/* a mask for signals */
...
extern int siggetmask (void) __THROW __attribute__((deprecated("use 'sigprocmask' instead")));
