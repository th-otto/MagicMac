...
#ifndef SEEK_SET		/* <stdio.h> has the same definitions.  */
...
extern void _exit (int __status) __NORETURN;
...
extern int getpagesize (void)  __THROW __attribute_const__;
...
/* Hm, syscall() must return int, but some of the functions it calls
   ...
   In newer versions of GLIBC, it is declared as returning long int,
   but we might as well keep this definition.
   */
extern long int syscall	(int, ...) __THROW;
#else
...
/*
   ...
   but we might as well keep this definition.
*/
extern int syscall (int, ...) __THROW;
#endif
...
/* return a username associated with the effective user ID of the process */
#ifndef L_cuserid
#define L_cuserid 80
#endif
