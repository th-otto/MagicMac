...
static int delete_printers(PRN_ENTRY **printers);
static int free_list(void **root);


#define P_COOKIES ((long *) 0x5a0)

static int delete_sub_dialogs(PDLG_SUB **sub);
static int delete_paper_types(MEDIA_TYPE **types);
static int delete_paper_sizes(MEDIA_SIZE **sizes);
static int delete_trays(PRN_TRAY **trays);
...
static VDIPB vq_ext_pb = { _myVDIParBlk.control, _myVDIParBlk.intin, _myVDIParBlk.xptsin, _myVDIParBlk.xintout, _myVDIParBlk.xptsout };
...
static int delete_sub_dialogs(PDLG_SUB **list)
{
	while (*list != NULL)
	{
		PDLG_SUB *sub = *list;
...
#if BINEXACT
							entry->offset_hdr = (long)hdr - (long)buf;
#else
...
/* FIXME: use reentrant version */
WORD vq_ext_devinfo(WORD handle, WORD device, WORD *dev_exists, char *filepath, char *filename, char *name)
{

#if BINEXACT
	(void) "A:\\ASSIGN.SYS\0.SYS\0SYS\0.SYS\0SYS\0PATH";
#endif

	_myVDIParBlk.control[0] = 248;
	_myVDIParBlk.control[1] = 0;
	_myVDIParBlk.control[3] = 7;
	_myVDIParBlk.control[5] = 4242;
	_myVDIParBlk.control[6] = handle;
	_myVDIParBlk.intin[0] = device;
	*((char **)&_myVDIParBlk.intin[1]) = filepath;
	*((char **)&_myVDIParBlk.intin[3]) = filename;
	*((char **)&_myVDIParBlk.intin[5]) = name;
	vdi(&vq_ext_pb);
	_myVDIParBlk.control[5] = 0;
	*dev_exists = _myVDIParBlk.xintout[0];
	return _myVDIParBlk.xintout[1];
}
