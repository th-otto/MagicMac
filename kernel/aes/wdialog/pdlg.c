...
WORD pdlg_do(PRN_DIALOG *prn_dialog, PRN_SETTINGS *settings, const char *document_name, WORD option_flags)
{
	...
	trees = prn_dialog->tree_addr; /* BINEXACT: unneeded */
	size = tree_memsize(trees[MAIN_DIALOG]);
...
#if BINEXACT
	{
		WORD volatile sel = item->selected;
		if (sel);
		nop();
	}
#endif
...
#define NUM(x) (WORD)(sizeof(x) / sizeof(x[0]))
		...
		{
			OBJECT *obj = &tree[icons[i]]; /* FIXME: BINEXACT */
