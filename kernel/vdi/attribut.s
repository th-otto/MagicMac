...
vswr_mode_set: 	move.w	d0,(a0)			;intout[0] = Grafikmodus;
						...
						bhi.s 	vswr_mode_err
						rts
vswr_mode_err: 	moveq 	#REPLACE,d0 	;intout[0] = REPLACE;
						bra.s 	vswr_mode_set
...
vsl_type_set:		move.w	d0,(a0)			;intout[0] = neuer Linienstil;
						...
						bhi.s 	vsl_type_err
						rts
vsl_type_err:		moveq 	#L_SOLID,d0 	;intout[0] = L_SOLID;
						bra.s 	vsl_type_set

; SET USER-DEFINED LINE STYLE PATTERN (VDI 113)

vsl_udsty:			movea.l	pb_intin(a0),a1 ;intin
						move.w	(a1),l_sdstyle(a6) ;eigenen Linienstil eintragen
						rts
...
vsl_width_min: 	tst.w 	d0
						bpl.s 	vsl_width_max
						moveq 	#L_WIDTH_MIN,d0
						bra.s 	vsl_width_set
vsl_width_max: 	moveq 	#L_WIDTH_MAX,d0
						bra.s 	vsl_width_set
...
vsl_color_set: 	move.w	d0,(a0)			;intout[0] = neue Linienfarbe
						...
						rts
vsl_color_err: 	moveq 	#BLACK,d0		;intout[0] = BLACK;
						bra.s 	vsl_color_set
...
vst_height6:		movea.l	pb_ptsout(a0),a1 ;ptsout
						move.l	#$070006,d0		;Zeichenbreite/Zeichenhoehe
						movea.l	#$080008,a0		;Zeichenzellenbreite/-hoehe
						...
						lea		t_base(a6),a1
						move.l	#$060002,(a1)+ ;Oberkante<->Basislinie/Halblinie
						moveq		#7,d0
						move.l	d0,(a1)+			;Oberkante<->B.-Oberkante/Unterkante
						swap		d0
						move.l	d0,(a1)+			;Oberkante<->B._Unterkante/Oberkante
						addq.l	#(t_left_off-t_top-2),a1
						move.l	#$010008,(a1)+ ;t_left_off/t_whole_off linker Offset/Verbreiterung
						moveq 	#0,d0
						move.l	d0,(a1)+ 		;t_thicken/t_uline fett/unterstrichen
						move.w	d0,t_prop(a6)	;t_prop/t_grow
						lea		font_hdr2,a0	;Adresse des Fontheaders
						lea		t_fonthdr(a6),a1
						move.l	a0,(a1)+ 		;t_fonthdr
						move.l	a0,CUR_FONT.w	;Kompatibilitaet
						lea		off_table(a0),a0
						move.l	(a0)+,(a1)+ 	;t_offtab
						move.l	(a0)+,(a1)+ 	;t_image
						move.l	(a0)+,(a1)+ 	;t_iwidth/t_iheight
						rts

vst_height13:		movea.l	pb_ptsout(a0),a1 ;ptsout
						move.l	#$07000d,d0		;Zeichenbreite/Zeichenhoehe
						movea.l	#$080010,a0		;Zeichenzellenbreite/-hoehe
						move.l	d0,(a1)+			;Zeichenbreite/Zeichenhoehe
						move.l	a0,(a1)+			;Zeichenzellenbreite/-hoehe
						lea		t_width(a6),a1
						move.l	d0,(a1)+			;t_width / t_height
						move.l	a0,(a1)+			;t_cwidth / t_cheight
						lea		t_base(a6),a1
						move.l	#$0d0005,(a1)+ ;Oberkante<->Basislinie/Halblinie
						move.l	#$02000f,(a1)+ ;Oberkante<->B.-Oberkante/Unterkante
						move.l	#$0f0000,(a1)+ ;Oberkante<->B.-Unterkante/Oberkante
						addq.l	#(t_left_off-t_top-2),a1
						moveq 	#0,d0
						move.l	#$010008,(a1)+ ;t_left_off/t_whole_off linker Offset/Verbreiterung
						move.l	d0,(a1)+ 		;t_thicken/t_uline fett/unterstrichen
						move.w	d0,t_prop(a6)	;t_prop/t_grow
						lea		font_hdr3,a0 	;Adresse des Fontheaders
						lea		t_fonthdr(a6),a1
						move.l	a0,(a1)+ 		;t_fonthdr
						move.l	a0,CUR_FONT.w	;Kompatibilitaet
						lea		off_table(a0),a0
...
vst_h_calc: 		move.l	a0,t_fonthdr(a6) ;Adresse des Fonts
						...
						movem.w	first_ade(a0),d2-d3/d6 ;erstes/letztes Zeichen/Hoehe
...
vst_h_space:		move.w	d0,t_space_index(a6)
						...
						mulu		d7,d2
						mulu		d7,d3
						divu		d6,d0 			;linker Offset bei Kursivschrift
						divu		d6,d2 			;Verbreiterung bei Fettschrift
						divu		d6,d3 			;Breite der Unterstreichung
...
vst_h_offsets: 	movem.w	d0-d3,t_left_off(a6)	;t_left_off / t_whole_off / t_thicken / t_uline
						...
						mulu		d7,d1
						mulu		d7,d3
						divu		d6,d1 			;neue Zeichenbreite
						divu		d6,d3 			;neue Zeichenzellenbreite
vst_h_ptsout:		movem.w	d1-d4,(a2)		;ptsout[0...1]
						...
						moveq 	#0,d5 			;Oberkante<->Oberkante
						cmp.w 	d6,d7 			;Vergroesserung ?
						beq.s 	vst_h_exit
						mulu		d7,d1
						mulu		d7,d2
...
vst_rot_pos:		addi.w	#450,d0			;Rundung !
						divu		#900,d0			;0=>0 degree, 1=>90 degree, 2=>180 degree, 3=>270 degree
...
vst_font:			movea.l	pb_intin(a0),a1
						...
						lea		font_hdr1,a0 ;Header des ersten Systemfonts
...
vst_color_set: 	move.w	d0,(a0)			;intout[0] = neue Linienfarbe
						...
						rts
vst_color_err: 	moveq 	#BLACK,d0		;intout[0] = BLACK;
						bra.s 	vst_color_set

; SET GRAPHIC TEXT SPECIAL EFFECTS (VDI 106)
vst_effects:		movea.l	pb_intin(a0),a1 ;intin
						movea.l	pb_intout(a0),a0 ;intout
						moveq 	#T_OUTLINED+T_UNDERLINED+T_ITALICS+T_LIGHT+T_BOLD,d0
...
vsf_int_set:		move.w	d0,(a0)			;intout[0] = neuer Fuelltyp;
						...
						lea		f_planes(a6),a0
						clr.w		(a0)
						move.b	vsf_int_tab4(pc,d0.w),d0
...
vsf_int_tab4:		DC.B vsf_int_user_def-vsf_int_user_def
						DC.B 0
...
vsf_sty_pat_save:	move.w	d0,(a0)			;intout[0] = neuer Musterindex;
						...
						rts
vsf_sty_pat_err:	moveq 	#1,d0 			;Musterindex = 1;
...
vsf_sty_hat_save:	move.w	d0,(a0)			;intout[0] = neuer Musterindex;
						...
						move.l	a0,f_pointer(a6)
						rts
vsf_sty_hat_err:	moveq 	#1,d0 			;Musterindex = 1;
...
vsf_color_set: 	move.w	d0,(a0)			;intout[0] = neue Musterfarbe
						...
						rts
vsf_color_err: 	moveq 	#BLACK,d0		;intout[0] = BLACK;
						bra.s 	vsf_color_set
