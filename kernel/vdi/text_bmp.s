...
text_part_line:
	moveq.l  #15,d5                     /* number of lines - 1 */
	sub.w    d0,d1
	cmp.w    d5,d1
...
textp_rot90:
	subq.w   #T_ROT_90,d7               /* rotate 90 degrees ? */
	bne.s    textp_rot180
...
textp_rot180:
	subq.w   #T_ROT_180-T_ROT_90,d7     /* rotate 180 degrees ? */
	bne.s    textp_rot270
	movem.l  d6/a3/a5,-(sp)
...
text_part_skew180:
	move.w   #$5555,d6                  /* t_skew_mask BUG: should be taken from fonthdr */
	move.w   d5,d7
	move.w   d5,d1
textp_ital_loop180:
	moveq.l  #0,d5
 ...
 */
text:
	move.w   v_nintin(a1),d6            /* character count */
...
text_eff_out:
	btst     #T_OUTLINED_BIT,d0         /* outlined? */
	beq.s    text_thicken
	addq.w   #2,d5                      /* thickening for outline */
text_thicken:
	move.w   d5,t_eff_thicken(a6)       /* widening for effects */
...
text_width_add:
	add.w    d5,d3                      /* + widening from effects */
	add.w    d3,d4
	dbra     d7,text_width
...
text_mono:
	move.w   t_cwidth(a6),d4            /* character width */
...
text_clip_rot180:
	subq.w   #T_ROT_180-T_ROT_90,d7     /* 180 degreee ? */
	bne      text270
...
text_cl00_x1:
	sub.w    d2,d0                      /* x1 of text rectangle */
...
text_cl0_x1:
	cmp.w    clip_xmin(a6),d0           /* clip left ? */
	bge      text_cl0_x2
...
text_cl0_x2:
	cmp.w    clip_xmax(a6),d2           /* clip right ? */
	ble      text_cl0_width
...
text_buf_size:
	mulu.w   d4,d7                      /* needed buffer size */
	...
	beq.s    text_buf_cmp
text_buf_shrink:
	lsr.l    #1,d4                      /* halved due to certain effects */
text_buf_cmp:
	cmp.l    d4,d7                      /* does everything fit into buffer ? */
	bgt      text_partitial
 ...
 */
underline:
	move.w   t_act_line(a6),d0          /* start line of section */
	move.w   d0,d1
	...
	move.w   d7,d3
underline_width:
	add.w    d3,d2                      /* end line of underline */
	cmp.w    d7,d2
	ble.s    underline_top
	move.w   d7,d2
underline_top:
	cmp.w    d1,d3                      /* too far to top ? */
...
bold_loop:
	move.w   d6,d7                      /* counter inside line */
 ...
 */
light:
	move.w   #$5555,d0                  /* mask for light font (t_light_mask) */
	moveq.l  #15,d6
...
light_loop2:
	...
	rts
light_italics:
	move.w   #$5555,d2                  /* t_skew_mask(a6) shift mask for italics */
	ror.w    d6,d2                      /* mask according to relativ line */
light_i_loop1:
	move.w   d1,d6                      /* word counter */
light_i_loop2:
	and.w    d0,(a3)+
...
rotate90_bloop:
	move.w   d4,d3                      /* pixel counter */
	movea.l  a0,a4
	movea.l  a1,a5
	adda.w   a2,a0
	bra.s    rotate90_read
rotate90_loop:
	dbra     d1,rotate90_test
rotate90_read:
	moveq.l  #15,d1                     /* bit counter */
...
rotate180_bloop:
	move.w   a4,d2                      /* dot mask */
	move.w   d4,d3                      /* counter */
	moveq.l  #0,d7
	bra.s    rotate180_read
rotate180_loop:
	dbra     d1,rotate180_test
rotate180_read:
	moveq.l  #15,d1                     /* bit counter */
	move.w   (a0)+,d0                   /* source word */
...
rotate180_dbra:
	dbra     d3,rotate180_loop
rotate180_next:
	dbra     d5,rotate180_bloop
...
rotate270_test:
	...
	or.w     d2,(a5)                    /* set bit */
rotate270_white:
	adda.w   a3,a5                      /* next dest line */
	dbra     d3,rotate270_loop
rotate270_shift:
	suba.w   a2,a0                      /* next source line */
	ror.w    #1,d2                      /* next dot mask */
	bcc.s    rotate270_next
	addq.l   #2,a1                      /* next dest word */

rotate270_next:
	dbra     d5,rotate270_bloop
...
textblt_xs0:
	moveq.l  #0,d0
textblt_ys0:
	moveq.l  #0,d1                      /* y-Quellkoordinate = 0 */
textblt:
	move.w   d5,d7
	add.w    d2,d4                      /* xd2 */
	add.w    d3,d5                      /* yd2 */
 ...
 */
fill_text_buf:
	movea.w  t_iwidth(a6),a2            /* width of the font images */
	movea.l  t_offtab(a6),a4            /* character offset table */

ftb_eff:
	move.w   a3,d0                      /* buffer width in bytes */
...
ftb_no_offset:
	cmp.w    d7,d2                      /* more than 15 shifts ? */
	ble.s    ftb_next
	move.w   d2,d4
	lsr.w    #4,d4
	add.w    d4,d4
...
ftb_next:
	dbra     d6,ftb_loop                /* copy next character */
	move.l   a1,d4
 ...
 */
text_offset:
	move.w   d6,d0                      /* remaining character count */
...
ftb_grow_next:
	dbra     d6,ftb_grow_loop           /* copy next character */
	move.l   a1,d4
	sub.l    buffer_addr(a6),d4
	lsl.w    #3,d4
	add.w    d2,d4
...
grow_byte_loop:
	add.w    d1,d1                      /* make room */
	add.w    d1,d1
	add.b    d0,d0                      /* bit set ? */
	bcc.s    grow_byte_next
	addq.w   #3,d1                      /* set 2 bits */
grow_byte_next:
	dbra     d3,grow_byte_loop
...
grow_char2:
	...
	beq.s    grow_byte2
grow_char_db:
	move.w   d2,d3                      /* number of dest shifts */
 ...
 */
shrink_char:
	addq.w   #1,d5                      /* buffer height in pixel */
	move.w   t_cheight(a6),d7
...
textj_thicken:
	move.w   d5,t_eff_thicken(a6)       /* widening from effects */
...
textj_add:
	add.w    d5,d2                      /* + widening from effects */
	add.w    d2,d4
