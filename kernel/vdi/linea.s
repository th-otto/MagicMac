...
linea_tab:        DC.L linea_init         ;A-init           ($a000)
                  ...
                  DC.L transform_mouse    ;Transform mouse  ($a00b)
...
linea_col_next:   dbra     d1,linea_color
                  ...
                  bne.s    linea_color16
linea_col_black:  moveq.l  #1,d0          ;VDI-Farbe 1
                  rts
linea_color16:    lea.l    color_remap_tab,a0
                  move.b   0(a0,d0.w),d0
                  rts
...
get_linea_pat:    movea.l  (PATPTR).w,a0    ;Musterzeiger
                  ...
                  movea.l  (buffer_ptr).w,a1     ;Buffer fuers Muster
                  move.w   (PATMSK).w,d0
...
get_lpat_mono2:   move.w   (a2)+,(a1)+
                  ...
                  movea.l  (buffer_ptr).w,a0     ;Buffer fuers Muster
                  moveq.l  #0,d0                
                  bra.s    get_lpat_call
get_lpat_color:   move.w   r_planes(a6),d0  ;Ebenenzaehler
...
linea_textblt:    move.l   a6,-(sp)
                  ...
                  movea.l  (linea_wk_ptr).w,a6              ;LineA-Workstation
...
transform_mouse:  movea.l  (INTIN).w,a2
transform_in:     move.w   (DEV_TAB13).w,d5
                  subq.w   #1,d5          ;hoechste Farbnummer
                  lea.l    -44(sp),sp     ;statt movem d1-d7/a2-a5
                  bra      vsc_form_in2
...
undraw_sprite:    move.l   (undraw_spr_vec).w,-(sp)
                  rts
undraw_sprite_in: move.w   (a2)+,d2
                  subq.w   #1,d2          ;Sprite-Zeilen - 1 wg. dbf
                  bmi.s    undraw_exit

                  cmpi.w   #30,(nvdi_cpu_type).w    ;68030?
                  bne.s    undraw_spr_addr
                  btst     #0,(blitter+1).w   ;Blitter eingeschaltet?
...
vbl_mouse:        tst.w    (M_HID_CNT).w  ;Darf die Maus gezeichnet werden ?
                  ...
                  lea.l    (M_POS_HX).w,a0  ;Adresse des Mauszeigers
...
draw_sprite:      move.l   (draw_spr_vec).w,-(sp)
                  rts
...
Xko_lt_intxh:     addi.w   #16,d0         ;X_koord+16
                  moveq.l  #4,d2          ;Offset fuer MOVE-Routinen
                  bra.s    get_yhot

X_am_rRand:       moveq.l  #8,d2
get_yhot:         sub.w    2(a0),d1       ;Y_Koord - intyhot
                  lea.l    10(a0),a0      ;Zeiger auf das Sprite-Image
...
calc_addr:        move.w   d0,-(sp)
                  ...
                  and.w    #$fff0,d0
                  asr.w    #3,d0
                  mulu.w   (PLANES).w,d0  ;Anzahl der Bytes
