 ...
 */
set_xbios_res:
	cmp.w      #FALCONMDS+1,d3          /* Falcon-Modus? */
	bne.s      set_xbios_rsave
	cmpi.w     #FALCON_VIDEO,nvdi_cookie_VDO     /* Falcon? */
...
set_xbios_rsave:
	...
	cmpi.w     #TT_VIDEO,nvdi_cookie_VDO         /* TT ? */
	bne.s      set_res_mono
	moveq.l    #TT_HIGH+1,d1            /* hohe TT-Aufloesung (Xbios) */
set_res_mono:
	cmp.w      d1,d0                    /* momentan Monochrombetrieb ? */
	beq.s      set_res_exit
...
set_falcon_res:
	...
	move.w     #SETSCREEN,-(sp)         /* VsetScreen */
	trap       #14
 ...
 */
opnwk_load_drvr:
	bsr        set_xbios_res  /* XBIOS-Aufloesung setzen => Treiber wird geladen */
...
v_opnwk:
	...
	movem.l    (a0),a1-a5               /* a1=control, a2=intin, a3=ptsin, a4=intout, a5=ptsout */
...
v_opnwk_eddi:
	move.l     #0x45644449,d0           /* 'EdDI' Offscreen-Funktionen sind vorhanden */
 ...
 */
alloc_wk:
	moveq.l    #0,d4          /* Vorbesetzung mit Handle 0 */
	moveq.l    #MAX_HANDLES-2,d2 /* Zaehler fuers Handle */
	lea.l      wk_tab+4,a6    /* Zeiger auf die Workstationtabelle, ersten Eintrag ignorieren */
opnwk_loop:
	cmpi.l     #closed,(a6)+  /* Eintrag frei ? */
	dbeq       d2,opnwk_loop
	...
	bra.s      alloc_wk_exit
opnwk_all_len:
	move.l     #WK_LENGTH,d0  /* minimale Laenge der WK */
...
free_wk:
	lsl.w      #2,d0
	lea.l      wk_tab0,a0               /* Zeiger auf die Workstationtabelle */
 ...
 */
opnwk_io:
	bsr        init_arrays              /* LINE-A-Variablen setzen */
...
init_fonthdr:
	...
	lea.l      font_hdr1,a1             /* 6*6  Fontheader */
	lea.l      font_hdr2,a2             /* 8*8  Fontheader */
	...
	move.l     dat_table(a2),(V_FNT_AD).w     /* address of font image */
	...
	lea.l      font_hdr3,a2             /* 8*16 Fontheader */
init_nvdi_exit:
	move.l     a2,(DEF_FONT).w          /* Zeiger auf den Systemfont */
	move.l     dat_table(a2),(V_FNT_AD).w  /* address of font image */
...
init_interrupts:
	...
	lea.l      old_etv_timer,a1
	bsr        change_vec

	move.l     #just_rts,(USER_TIM).w
	move.l     old_etv_timer,(NEXT_TIM).w
	move.l     #just_rts,(USER_BUT).w
	move.l     #user_cur,(USER_CUR).w
	move.l     #just_rts,(USER_MOT).w
...
mouse_form:
	.dc.w 1                  /* mf_xhot */
	.dc.w 1                  /* mf_yhot */
	.dc.w 1                  /* mf_nplanes - always 1 */
	.dc.w 0	/* mf_fg - mask color */
	.dc.w 1	/* mf_bg - cursor color */
	/* mask */
	.dc.w 0xc000
	.dc.w 0xe000
	.dc.w 0xf000
...
wk_defaults:
	...
	cmpa.l     aes_wk_ptr,a6            /* first WK ? */
	beq.s      wk_arrays
	movea.l    aes_wk_ptr,a0
	move.w     res_ratio(a0),res_ratio(a6)
wk_arrays:
	move.b     #0x0f,input_mode(a6)     /* all I_SAMPLE */
/* bezier */
...
/* fill attributes */
	move.w     #1,f_perimeter(a6)       /* perimeter on */
	lea.l      f_saddr(a6),a0
...
wkdef_off:
	move.l     a1,d0                    /* Offscreen-Treiber vorhanden? */
	beq.s      wkdef_drv
...
v_opnvwk:
	...
	beq.s      v_opnbm
v_opnvwk_id:
	move.w     driver_id(a6),d3         /* Geraetekennung */
	move.l     device_drvr(a6),a3
...
v_opnbm_err:
	move.l     (sp),a0                  /* pb */
	move.l     pb_control(a0),a1        /* contrl */
	clr.w      v_handle(a1)
	movem.l    (sp)+,d1-d7/a2-a5
	rts
...
v_clswk_next:
	...
	move.l     #0x45644449,d0        /*  'EdDI' Offscreen-Funktionen sind nicht mehr vorhanden */
	bsr        reset_cookie
v_clswk_exit:
	movem.l    (sp)+,d1-d3/a2
	rts
...
v_clsvwk_err:
	movem.l    (sp)+,d1-d2/a2
	cmp.w      #AES_HANDLE,d0           /* statische Workstation ? */
...
v_clrwk:
	...
	move.l     f_fill0(a6),f_pointer(a6) /* empty fillpattern */
	clr.w      f_planes(a6)             /* monochrome */
	clr.w      f_interior(a6)           /* white */
	moveq.l    #0,d0
	moveq.l    #0,d1
	move.w     res_x(a6),d2             /* width - 1 */
	move.w     res_y(a6),d3             /* height - 1 */
	move.l     a6,-(sp)
	bsr        fbox_noreg
	movea.l    (sp)+,a6
 ...
 */
v_updwk:
	rts	                                /* Funktion ist beim Bildschirm unnoetig */
...
vst_lf_in:
	...
	moveq.l    #0,d1
vst_lf_loop:
	cmp.w      font_id(a0),d0           /* gleiche Nummer ? */
	beq.s      vst_lf_format
	move.w     font_id(a0),d0
	addq.w     #1,d1                    /* Anzahl inkrementieren */
vst_lf_format:
	tst.b      flags+1(a0)              /* Motorola- oder Intel- Format ? BUG: should btst #T_SWAP_BIT */
	bne.s      vst_lf_mot
...
vst_lf_swap:
	...
	dbf        d2,vst_lf_swap
vst_lf_mot:
	bset       #T_SWAP_BIT,flags+1(a0)  /* Motorola-Format */
 ...
 */
vs_clip:
	movem.l    pb_intin(a0),a0-a1       /* intin/ptsin */
