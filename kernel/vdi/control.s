...
set_xbios_rsave:	movem.l	d0-d1,-(sp)
						...
						beq.s 	set_res_exit
						moveq 	#ST_HIGH+1,d1	;hohe ST-Aufloesung (Xbios)
						cmpi.w	#TT_VIDEO,video ;TT ?
						bne.s 	set_res_mono
						moveq 	#TT_HIGH+1,d1	;hohe TT-Aufloesung (Xbios)
...
set_falcon_res:	movem.l	d0-d2/a0-a2,-(sp)
						...
						moveq 	#-1,d0			;Adressen nicht aendern
...
alloc_wk:			moveq 	#0,d4 			;Vorbesetzung mit Handle 0
						moveq 	#MAX_HANDLE-2,d2 ;Zaehler fuers Handle
...
v_opnwk_in: 		movem.l	d0-d1/a0-a1/a3,-(sp)
						...
						movea.l	d1,a0
						pea		opnwk_tcolor(pc)
						cmpi.w	#319,res_y(a6) ;mindestens 320 Pixel Hoehe?
						blt		vst_height6
...
init_res:			movem.l	d0-d2/a0-a2,-(sp)
						...
						lea		INQ_TAB0.w,a0	;intout fuer vq_extnd
						lea		-64(sp),sp
...
init_interrupts:	movem.l	d0-a6,-(sp)
						...
						clr.w 	MOUSE_BT.w		;keine Maustaste gedrueckt
						...
						pea		mouse_param(pc) ;Parameter fuer Initmous
...
;Eingaben:
;d1.l pb oder 0L
;a0.l Zeiger auf die Geraetetreiber-Struktur DEVICE_DRIVER
...
;Bezier
						move.w	#5,bez_qual(a6) ;Bezier maximale Qualitaet
						clr.l 	bez_buffer(a6) ;kein speziell Bezier-Puffer vorhanden
						clr.l 	bez_buf_len(a6) ;Laenge des Bezier-Buffers ist 0
;Clipping
...
wkdef_exit:			moveq		#0,d0
						...
						divu		d1,d0								;durch Pixelhoehe teilen
...
v_clswk_next:		dbra		d3,v_clswk_all
						...
						lea		nvdi_struct,a0		;NVDI-Struktur uebergeben
...
;Interrupt-Vektoren setzen
						...
						lea		USER_TIM.w,a0	;Line-A-Vektoren loeschen
						REPT 5
						clr.l 	(a0)+
						ENDM
						clr.l 	-(sp)
...
vst_lf_mot: 		bset		#2,flags+1(a0) ;Motorola-Format
						...
						move.l	t_bitmap_fonts(a6),FONT_RING2.w ;Kompatibilitaet
