...
init_interrupts:  movem.l  d0-d7/a0-a6,-(sp)
                  ...
                  lea.l    sys_timer(pc),a0 ;neue etv_timer-Routine
                  ...
                  moveq.l  #1,d0
                  move.b   d0,(M_HID_CNT).w ;Maus darf nicht gezeichnet werden
                  ...
                  pea.l    mouse_param(pc) ;Parameter fuer Initmous
                  moveq.l  #1,d0          ;Initmouse/relativer Mausmodus
                  move.l   d0,-(sp)
                  trap     #XBIOS
                  lea.l    12(sp),sp
                  move.w   (sp)+,sr
...
wk_defaults:      movem.l  d0-d2/a0-a1,-(sp)
                  ...
                  cmpa.l   (aes_wk_ptr).w,a6     ;erste WK ?
                  beq.s    wk_arrays
...
vst_lf_format:    tst.b    flags+1(a0)    ;Motorola- oder Intel- Format ? BUG: should btst #T_SWAP_BIT
                  bne.s    vst_lf_mot
; big nonsense: if the font is swapped, the header is swapped, too (and the bit to test is in flags, not flags+1)
