...
y_start:          .ds.w 1                 /* Y-Startkoordinate fuer die Linie */
x_end:            .ds.w 1                 /* X-Endkoordinate fuer die Linie */
y_end:            .ds.w 1                 /* Y-Endkoordinate fuer die Linie */
n_pts:            .ds.w 1                 /* nach Abtesten des ptsin[] aktuelle Koord.zahl */
_dx:              .ds.w 1
_dy:              .ds.w 1
_dx_sign:         .ds.w 1                 /* Vorzeichen: 0 pos, -1 neg. */
_dy_sign:         .ds.w 1
ptsin_chgd:       .ds.w 1                 /* Flag: ptsin-Koord. geaendert: Bit 0 Startk., Bit 1 Endk. */
sizeof_pts_start:
...
exit_vplth:
	lea.l      sizeof_pts_start(sp),sp  /* Stack korrigieren */
...
_fat_while:
	dbf        d3,fat_TT_LOW            /* bis v_nptsin < 2 */
...
_fat_qwhile:
	dbf        d3,_fat_qpix             /* bis v_nptsin < 2 */
...
gross_hyp:
	bsr.s      hypot                    /* Hypothenuse in d0.l zurueck */
...
sqrt:
	moveq.l    #0,d0                    /* xroot */
	move.l     #0x10000000,d2           /* m2 */

lblA:
	move.l     d0,d3                    /* x2 = xroot + */
	add.l      d2,d3                    /*      + m2 */
	lsr.l      #1,d0                    /* xroot/2 */
...
_st_ell_TT:
	asl.w      #1,d3                    /* TT-LOW Hoehe verdoppeln */

_strt_ell:
	movem.w    (a2),d0-d1               /* x- , y-Start */
...
_fwd_loop:
	moveq.l    #0,d6                    /* dx/dy- Vorzeichn pos. */
	moveq.l    #0,d7
...
_fwd_found:
	subq.l     #4,a0                    /* letzte Pos. des ptsin-Zeigers */
	move.l     a0,nxt_ptsin(a1)
...
exit_fwd:
	movem.w    d2-d3/d6-d7,_dx(a1)      /* _dx/_dy und dx/dy-Vorzeichen eintragen */
	addq.w     #2,d5
	move.w     d5,n_pts(a1)
	movem.l    (sp)+,d3-d7
	rts
...
v_pline1:
	...
	move.w     l_style(a6),d0           /* line style index */
	add.w      d0,d0
...
v_pline_ret2:
	...
	movem.l    (sp)+,d1-d7/a2-a3
v_pline_exit:
	clr.w      l_lastpix(a6)            /* 0, draw last point */
	rts
...
v_bez_loop:
	addq.w     #1,d2                    /* Zaehler inkrementieren */
...
v_bezarr_mask:
	...
	bra.s      v_bez_draw
v_bez_arrp:
	and.b      0(a5,d0.w),d3            /* Jump-Point oder Bezier? */
	beq        v_bez_next

v_bez_draw:
	addq.w     #1,d2                    /* Anzahl der Punkte im Linienzug */
	move.w     d2,d0
	movea.l    a4,a0
	add.w      d2,d2
	add.w      d2,d2
...
v_bez_lines:
	cmp.w      #2,d0                    /* weniger als zwei Punkte? */
	blt.s      v_bez_bez
...
v_bez_next:
	addq.l     #1,a5                    /* naechster bezarr-Eintrag */
	dbf        d7,v_bez_loop

v_bez_exit:
	move.l     (sp)+,l_start(a6)        /* l_start/l_end */
...
nvdi_lines:
	lea.l      -sizeof_PB-sizeof_contrl(sp),sp
...
nvdi_lines_ret:
	lea.l      sizeof_contrl+sizeof_PB(sp),sp
	rts
...
calc_bq_loop:
	cmp.w      -(a0),d0                 /* Laenge des Linienzugs ausreichend? */
	bge.s      calc_bez_qual
	dbf        d2,calc_bq_loop          /* Qualitaet herabsetzen */
	moveq.l    #0,d2
calc_bez_qual:
	movea.w    d2,a0                    /* neue Qualitaet */
...
calc_bez_addr:
	...
	move.w     #0x8000,d0               /* fuer die Rundung */
	move.w     d0,d1
	move.w     d0,d2
	move.w     d0,d3
	move.w     d0,d4
 ...
 */
generate_bez:
	cmpa.w     #0,a0                    /* Qualitaet = 0, Rekursionsende? */
	beq.s      bez_out
...
v_pmarker_loop2:
	...
	bsr        line
	movem.w    (sp)+,d0-d4
	dbf        d4,v_pmarker_loop2
	move.w     (sp)+,d5
	dbf        d5,v_pmarker_loop1
	lea.l      64(sp),sp
v_pm_exit:
...
v_gtext:
	movem.l    d1-d7/a2-a5,-(sp)
	movem.l    (a0),a1-a3               /* a1=control, a2=intin, a3=ptsin */
...
v_fillline:
	...
	move.w     d1,f_perimeter(a6)       /* perimeter on */
	clr.w      f_planes(a6)
	move.l     f_fill1(a6),f_pointer(a6) /* Zeiger auf fill_1 */
	movea.l    a0,a3                   /* ptsin */
	move.w     d0,d4
...
vfa_last_point:
	cmpa.l     d0,a4                    /* erster und letzer Punkt identisch? */
...
fpoly_count:
	sub.w      d5,d7                    /* Zeilenzaehler */
fpoly_loop:
	movea.l    a4,a0                    /* Quellbuffer */
 ...
 */
fpoly_hline:
	movea.l    a1,a3                    /* Zielbufferadresse sichern */
fpoly_calc:
	move.w     d5,d1
	move.w     (a0)+,d0                 /* x1 */
	sub.w      (a0)+,d1                 /* y - y1 */
...
fpoly_bubble2:
	...
	move.w     d2,(a1)
fpoly_bubble3:
	dbf        d0,fpoly_bubble2
	dbf        d1,fpoly_bubble1
...
v_bezf_saveq:
	move.w     bez_qual(a6),-(sp)       /* Bezier-Qualitaet sichern */
...
v_bezf_pts_nxt:
	dbf        d0,v_bezf_points

v_bezfq_calc:
	move.w     bez_qual(a6),d0          /* gewuenschte Bezier-Qualitaet */
	move.l     a5,d4
...
v_bezf_set_qual:
	move.w     d0,bez_qual(a6)          /* realisierbare Bezier-Qualitaet */
...
v_bezf_cmp:
	cmp.w      d0,d2                    /* Koordinatenpaare tauschen? */
	bge.s      v_bezf_dx_dy
	exg        d0,d2
...
gpoly_bubble2:
	...
	move.w     d2,(a1)
gpoly_bubble3:
	dbf        d0,gpoly_bubble2
	dbf        d1,gpoly_bubble1
...
v_pline_fill:
	subq.w     #2,d4                    /* weniger als zwei Punkte? */
	bmi.s      vpfl_ex
	move.w     l_color(a6),-(sp)
	move.w     f_color(a6),l_color(a6)
	cmpi.w     #MD_XOR-MD_REPLACE,wr_mode(a6)
