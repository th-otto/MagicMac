...
v_pline_thick: 	movem.l	d1-d5/a2-a5,-(sp)
						...
						moveq 	#0,d3 			;Voreinst.: ptsin unveraendert
						...
						moveq 	#1,d3 			;ptsin[0/1] veraendert
...
exit_vplth: 		lea		24(sp),sp		;Stack korrigieren
...
_fat_while: 		dbra		d3,fat_TT_LOW	;bis n_ptsin < 2
						lea		8+16(sp),sp 	;Stackk korrigieren
...
_pix2q_TTLOW:		cmp.w 	#TT_LOWRES,d0
						bne.s 	exit_pix2q
...
_q2pix_TTLOW:		cmp.w 	#TT_LOWRES,d0
						bne.s 	exit_q2pix
...
gross_hyp:			bsr.s 	hypot 			;Hypothenuse in d0.l zurueck
						...
						mulu		d3,d5 			;dy * l_width = t_x
						mulu		d3,d4 			;dx * l_width = t_y

						divu		d0,d5 			;t_x/hyp = t_x
						lsr.w 	#1,d5
						ext.l 	d5
...
calc_dx_was:		divu		d0,d4 			;t_y/hyp = t_y
						lsr.w 	#1,d4
						ext.l 	d4
...
_strtfm_ARR:		cmp.w 	#ARROW,d0
						...
						bgt.s 	_strtfm_TTl
						moveq 	#9,d2 			;Pfeilhoehe 9 Pixel vorgeben
						bra.s 	_strtfm_calc
...
_strtfm_calc:		move.w	d2,d3
						mulu		_dx(a3),d2		;dx * l_width * 3
						mulu		_dy(a3),d3		;dy * l_iwdth * 3
						divu		d0,d2 			;= t_x
						divu		d0,d3 			;= t_y
...
strt_dy_pos:		lea		-16(sp),sp		;int[8]
...
_fwd_loop:			moveq 	#0,d6 			;dx/dy- Vorzeichn pos.
						moveq 	#0,d7
						...
						moveq 	#$ffffffff,d6	;dx-Vorzeichen neg.
_fwd_posdx:
						sub.l 	d1,d3 			;dy
						bpl.s 	_fwd_posdy
						neg.l 	d3
						moveq 	#$ffffffff,d7	;dy-Vorzeichen neg.
...
_bk_loop:			moveq 	#0,d6 			;dx/dy- Vorzeichn pos.
						moveq 	#0,d7
...
v_pline1:			movem.l	d2-d7,-(sp)
						pea		v_pline_ret1(pc) ;Ruecksprungadresse
...
;a0.l ptsin
;a3.l ptsout
search_min_max:	movem.l	d0/d2-a0,-(sp)
						subq.w	#1,d0
						movem.w	(a3),d4-d7
...
v_bez:				movem.l	d1-d7/a2-a5,-(sp)
						...
						moveq 	#0,d5 			;Punktzaehler insgesamt
						moveq 	#0,d6 			;Zaehler fuer Jump-Points
...
v_bez_draw: 		addq.w	#1,d2 			;Anzahl der Punkte im Linienzug
						...
						beq.s 	v_bez_lines
						moveq 	#0,d2
						subq.l	#4,a4 			;letzen Punkt als neuen Startpunkt benutzen
...
v_bez_save: 		movem.w	d5-d7,-(sp)
						...
						lea		1024(a2),a1 	;Buffer fuer Bezier Koordinaten
						...
						lea		8(a4),a4 		;2 Koordinatenpaare weiter
...
nvdi_lines: 		lea		-sizeof_pb-sizeof_contrl(sp),sp
						lea		sizeof_pb(sp),a1		;contrl
...
nvdi_lines_ret:	lea		sizeof_contrl+sizeof_pb(sp),sp
						rts

gdos_lines: 		movem.l	d2/a2,-(sp)
						lea		-sizeof_pb-sizeof_contrl-64(sp),sp	;Platz fuer lokalen pb und contrl
						lea		sizeof_pb(sp),a1		;contrl
						move.l	sp,d1						;pb
						...
						lea		sizeof_pb+sizeof_contrl+64(sp),sp	;Platz fuer pb und contrl zurueckgeben
						movem.l	(sp)+,d2/a2
						rts
...
v_pmarker_loop2:	movem.w	d0-d4,-(sp)
						...
						add.w 	(a0)+,d2
						add.w 	(a0)+,d3
						moveq 	#$ffffffff,d6	;durchgezogene Linie
						bsr		line
						movem.w	(sp)+,d0-d4
						dbra		d4,v_pmarker_loop2
						move.w	(sp)+,d5
						dbra		d5,v_pmarker_loop1
						lea		64(sp),sp		;Stack korrigieren
...
v_bezf_mem: 		move.l	d7,d1
						...
						divu		#9,d7 			;maximale Koordinatenanzahl
...
v_bezf_saveq:		move.w	bez_qual(a6),-(sp) ;Bezier-Qualitaet sichern
						...
						lea		-1024(a2),a2	;Buffer fuer Bezier-Generator
						movea.l	a2,a5
						move.w	d7,d0
						addq.w	#1,d0
						and.w 	#$fffe,d0
...
v_bezf_swap:		move.w	(a0)+,d1
						...
						moveq 	#0,d3 			;Zaehler fuer Beziers

v_bezf_points: 	moveq 	#1,d1
...
v_bezfq_calc:		move.w	bez_qual(a6),d0 ;gewuenschte Bezier-Qualitaet
						...
						lea		bez_pnt_tab(pc),a0 ;Tabelle mit Punktanzahlen der Bezier-Qualitaeten
...
;Ausgaben:
;a1.l ptsout[0..3]
fsearch_min_max:	movem.l	d0-a0,-(sp)
						subq.w	#1,d0
...
gpoly_loop: 		movea.l	a4,a0 			;Quellbuffer
						...
						lea		-sizeof_pb-sizeof_contrl-64(sp),sp	;Platz fuer pb und contrl
						lea		sizeof_pb+sizeof_contrl(sp),a2
						lea		sizeof_pb(sp),a1
						movea.l	sp,a0
						move.l	a1,(a0)+					;pb_contrl
...
gpoly_border:		lea		sizeof_pb(sp),a1
						...
						lea		sizeof_pb+sizeof_contrl+64(sp),sp
...
;Ausgaben:
;a6.l Workstation
gdos_fline: 		lea		-sizeof_pb-sizeof_contrl-64(sp),sp
						lea		sizeof_pb+sizeof_contrl(sp),a2	;ptsin
						lea		sizeof_pb(sp),a1		;contrl
						movea.l	sp,a0						;pb
						move.l	a1,(a0)+					;pb_contrl
...
gdos_get_lattr:	movem.l	d0-d2/a0-a2,-(sp)
						lea		-sizeof_pb-sizeof_contrl-64(sp),sp	;Platz fuer lokalen pb, contrl, intout, ptsout
						lea		sizeof_pb(sp),a1		;contrl
						move.l	sp,d1						;pb
						movea.l	d1,a0						;pb
...
gdos_line_std: 	movem.l	d0-d2/a0-a4,-(sp)
						lea		-sizeof_pb-sizeof_contrl-64(sp),sp	;Platz fuer lokalen pb, contrl, intin, ptsin, intout, ptsout
						lea		sizeof_pb(sp),a3		;contrl
						move.l	sp,d1						;pb
						movea.l	d1,a0						;pb
						
						move.l	a3,(a0)+					;pb_contrl
						lea		sizeof_contrl(a3),a4	;intin/ptsin
						move.l	a4,(a0)+					;pb_intin
						move.l	a4,(a0)+					;pb_ptsin
						lea		32(a4),a2				;intout/ptsout
						move.l	a2,(a0)+					;pb_intout
						move.l	a2,(a0)					;pb_ptsout
...
v_bez_on_off:		movea.l	pb_intout(a0),a0
						tst.w		n_ptsin(a1)
						bne.s		v_bez_on
						clr.w		bez_on(a6)
						clr.w		(a0)						
