...
vrq_choice: 		bsr.s 	v_input			;Zeichen einlesen
						...
						subi.b	#59,d1			;Funktionstate von 1 bis 10 ?
						cmpi.b	#9,d1
						bhi.s 	v_choice_out
						addq.b	#1,d1
...
v_string:			movem.l	d1-d5/a2-a4,-(sp)
						...
						bpl.s 	v_string_pos
						neg.w 	d4
						moveq 	#$ffffffff,d3	;Scan-Code durchlassen !
...
vsc_form_fg:		cmp.w 	d5,d4 			;ungueltige Vordergrundfarbe ?
						...
						moveq 	#1,d4
vsc_form_colors:	moveq 	#15,d5
						and.w 	d5,d0 			;Hot-Spot-x
						and.w 	d5,d1 			;Hot-Spot-y
						movea.l	color_map_ptr,a1
						...
						movem.w	d0-a1/a4-a5,MASK_FORM.w
						movem.w	(a2)+,d0/d2/d4/d6/a0/a4
						movem.w	(a3)+,d1/d3/d5/d7/a1/a5
						movem.w	d0-a1/a4-a5,MASK_FORM+24.w
						...
						move		d0,sr
						subq.b	#1,MOUSE_FLAG.w ;Maus darf gezeichnent werden
						movem.l	(sp)+,d1-d7/a2-a5
						rts
...
vex_timv:			movea.l	(a0),a1			;contrl
						...
						move		sr,d0
						ori		#$0700,sr		;Interrupts sperren
						move.l	USER_TIM.w,18(a1) ;contrl[9/10] = USER_TIM;
						move.l	14(a1),USER_TIM.w ;USER_TIM = contrl[7/8];
						move		d0,sr 			;altes sr
...
;v_show_c_in:
						...
						bne.s 	v_show_c2
						tst.w 	M_HID_CT.w		;Maus schon an ?
						beq.s 	v_show_c_exit3
						move.w	#ON,M_HID_CT.w ;Maus an
v_show_c2:			cmpi.w	#ON,M_HID_CT.w ;darf die Maus gezeichnet werden ?
						bgt.s 	v_show_c_exit
						...
						movem.l	(sp)+,d1-d7/a2-a5
v_show_c_exit: 	subq.w	#1,M_HID_CT.w
						rts
v_show_c_exit2:	clr.w 	M_HID_CT.w		;Maus darf gezeichnet werden
v_show_c_exit3:	rts
...
vex_butv:			movea.l	(a0),a1			;contrl
						move.l	USER_BUT.w,18(a1) ;contrl[9/10] = USER_BUT;
						move.l	14(a1),USER_BUT.w ;USER_BUT = contrl[7/8];
						rts

; EXCHANGE MOUSE MOVEMENT VECTOR (VDI 126)
vex_motv:			movea.l	(a0),a1			;contrl
						move.l	USER_MOT.w,18(a1) ;contrl[9/10] = USER_MOT;
						move.l	14(a1),USER_MOT.w ;USER_MOT = contrl[7/8];
						rts

; EXCHANGE CURCOR CHANGE VECTOR (VDI 127)
vex_curv:			movea.l	(a0),a1			;contrl
						move.l	USER_CUR.w,18(a1) ;contrl[9/10] = USER_CUR;
						move.l	14(a1),USER_CUR.w ;USER_CUR = contrl[7/8];
						rts

; SAMPLE KEYBOARD STATE INFORMATION (VDI 128)
vq_key_s:			movea.l	pb_intout(a0),a1 ;intout
						movea.l	key_state,a0
						moveq 	#15,d0
						and.b 	(a0),d0
