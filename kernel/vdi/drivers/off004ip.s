...
;******************************************************************************;
;*                                                                            *;
;*                      4-Farb-Offscreen-Treiber fuer NVDI 4.1                *;
;*                                                                            *;
;******************************************************************************;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
work_out_int:     move.w   (a2)+,(a0)+
                  ...
                  move.w   #4,78-90(a0)   ;work_out[39]: 4 Farbabstufungen in der Palette
...
ext_out_int:      move.w   (a2)+,(a0)+
                  ...
                  move.w   #4,2-90(a0)       ;work_out[1]: 4 Farbabstufungen
...
wk_init:
                  move.w   #1,r_planes(a6)   ;Anzahl der Bildebenen -1
                  move.w   #3,colors(a6) ;hoechste Farbnummer
...
get_pixel_line:   add.l    d1,a0
                  ...
                  lsr.w    #2,d1
                  addq.w   #4,d1
                  adda.w   d1,a0          ;Zeiger hinter die hoechstwertigste Plane
                  moveq    #15,d1
                  not.w    d0
                  and.w    d0,d1          ;Bitnummer
                  moveq    #1,d2          ;Planezaehler
                  moveq    #0,d0
...
copy_inc_right:   cmp.w    #4,d6          ;zwei Startworte lesen?
                  ...
                  beq      copy_il_right
                  tst.w    d7
                  bne.s    copy_ilw_r     ;kein zusaetzliches Endwort lesen
...
copy_inc_fword_r: cmp.w    #-2,d1         ;nur ein Wort ausgeben?
                  beq      copy_iw_right
                  tst.w    d7
                  bne.s    copy_iww_r     ;kein zusaetzliches Endwort lesen
...
copy_inc_left:    cmp.w    #4,d6          ;zwei Startworte lesen?
                  ...
                  beq      copy_il_left
                  tst.w    d7
                  bne.s    copy_ilw_l     ;kein zusaetzliches Endwort lesen
...
copy_inc_fword_l: cmp.w    #-2,d1         ;nur ein Wort ausgeben?
                  beq      copy_iw_left
                  tst.w    d7
                  bne.s    copy_iww_l     ;kein zusaetzliches Endwort lesen
...
copyd_word_loop_3:move.l   -(a0),d6
                  not.l    d6
                  and.l    d2,d6
                  or.l     d2,-(a1)
                  eor.l    d6,(a1)
...
copyd_shift:      subq.w   #4,a3

                  cmpa.l   #blt_dec_l_tab,a4
                  bne      copyd_right
...
fbox_mp_laddr:    move.w   d1,d7
                  ...
                  subq.w   #4,d6
                  movea.w  d6,a3          ;Bytes zur naechsten Zeile
                  asr.w    #4,d0
                  move.w   d0,d6
                  lsl.w    #2,d6
...
fbox:             tst.w    f_planes(a6)   ;mehrere Musterebenen?
                  bne      fbox_mplane
                  movea.l  v_bas_ad.w,a1  ;Adresse des Bildschirms
                  move.w   BYTES_LIN.w,d6 ;Bytes pro Zeile
                  tst.w    bitmap_width(a6) ;Off-Screen-Bitmap?
...
fbox_build_masks: moveq    #15,d4
                  ...
                  add.w    d5,d5
                  lea      fbox_mask(pc),a0
                  move.l   0(a0,d4.w),d4  ;Startmaske
                  ...
                  and.w    d6,d2
                  asr.w    #2,d2
                  sub.w    d0,d2          ;Anzahl der Bytes -4
...
fbox_white:       suba.w   d2,a3
                  subq.w   #4,a3
                  asr.w    #2,d2          ;Anzahl der Bytes pro Ebene -1
                  not.l    d4             ;Startmaske
...
fbox_usual:       move.w   a3,d0          ;Bytes pro Zeile
                  ...
                  sub.w    d2,d0
                  subq.w   #4,d0          ;- Laenge der Linie in Bytes
                  movea.w  d0,a2
                  asr.w    #2,d2          ;Anzahl der Bytes pro Ebene -1
...
fbox_replace_col: move.w   d1,-(sp)
                  ...
                  add.w    d1,d1
                  add.w    d1,d1
                  and.l    fbox_col_mask(pc,d1.w),d6
...
fbox_solid:       move.w   f_color(a6),d1
                  lea      color_map(pc),a4
                  moveq    #3,d6
                  and.b    0(a4,d1.w),d6
                  add.w    d6,d6
                  add.w    d6,d6
                  move.l   fbox_scol_mask(pc,d6.w),d6
...
fbox_solid_loop:  move.l   d6,(a4)+
                  dbra     d1,fbox_solid_loop
fbox_solid_last:  move.l   d6,d1
                  not.l    d1             ;Endmaske invertieren
                  and.l    d5,d1          ;Linienstil maskieren
                  or.l     d5,(a4)        ;Bildinhalt maskieren
                  eor.l    d1,(a4)+
...
fbox_trans_loop:  movea.l  a5,a4
                  ...
                  move.w   d2,d7
                  bmi.s    fbox_or_last
fbox_or_loop:     addq.l   #4,a4
                  or.w     d6,(a4)
                  dbra     d7,fbox_or_loop
fbox_or_last:     addq.l   #4,a4
                  move.w   d6,d7
                  and.w    d5,d7
                  or.w     d7,(a4)
...
fbox_and:         move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fbox_and_last
fbox_and_loop:    addq.l   #4,a4
                  and.w    d6,(a4)
                  dbra     d7,fbox_and_loop
fbox_and_last:    addq.l   #4,a4
                  not.w    d6
                  move.w   d6,d7
                  and.w    d5,d7
                  not.w    d7
...
fline_saddr_pat:  movem.l  a0/a2,-(sp)

                  movea.l  f_pointer(a6),a2
                  moveq    #15,d4
                  and.w    d1,d4
                  add.w    d4,d4
                  adda.w   d4,a2          ;Adresse der Fuellmusterzeile
...
fline_pat_screen: movea.l  v_bas_ad.w,a1  ;Adresse des Bildschirms
                  muls     BYTES_LIN.w,d1
...
fline_repl:       move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fline_r_last
fline_r_loop:     addq.l   #4,a0          ;naechstes Wort der Ebene
                  move.w   d6,(a0)
                  dbra     d7,fline_r_loop
fline_r_last:     addq.l   #4,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  not.w    d7
                  and.w    d4,d7
...
fline_or:         move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fline_or_last
fline_or_loop:    addq.l   #4,a0          ;naechstes Wort der Ebene
                  or.w     d6,(a0)
                  dbra     d7,fline_or_loop
fline_or_last:    addq.l   #4,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  and.w    d4,d7
                  or.w     d7,(a0)
...
fline_and_loop:   addq.l   #4,a0          ;naechstes Wort der Ebene
                  and.w    d6,(a0)
                  dbra     d7,fline_and_loop
fline_and_last:   addq.l   #4,a0          ;naechstes Wort der Ebene
                  swap     d4
                  not.w    d6
                  move.w   d6,d7
                  and.w    d4,d7
...
fline_eor_loop:   addq.l   #4,a0          ;naechstes Wort der Ebene
                  eor.w    d6,(a0)
                  dbra     d7,fline_eor_loop
fline_eor_last:   addq.l   #4,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  and.w    d4,d7
                  eor.w    d7,(a0)
...
hline_laddr:      adda.l   d1,a1          ;Zeilenadresse
                  ...
                  asr.w    #2,d2
                  sub.w    d0,d2          ;Anzahl der Bytes-4
                  asr.w    #2,d2          ;Anzahl der Worte pro Plane -1
                  ...
                  subq.w   #1,d2
                  bmi.s    hline_blackw1  ;Wort
                  subq.w   #1,d2
                  bmi.s    hline_blackw2  ;Worte
...
hline_eor:        subq.w   #1,d2
                  bmi.s    hline_eor_w1   ;Wort
                  subq.w   #1,d2
                  bmi.s    hline_eor_w2   ;Worte
...
hline_replace:    moveq    #0,d0
                  move.l   a1,d1          ;a1 sichern
                  lea      color_map(pc),a1
                  move.b   1(a1,d7.w),d0
                  movea.l  d1,a1          ;a1 zurueck
                  and.w    #3,d0
                  add.w    d0,d0
                  add.w    d0,d0
                  and.l    hline_col_mask(pc,d0.w),d6

                  subq.w   #1,d2
                  bmi.s    hline_replace_w1 ;Wort
                  subq.w   #1,d2
                  bmi.s    hline_replace_w2 ;Worte
...
hline_trans_loop: movea.l  a1,a0
                  ...
                  move.w   d2,d7
                  bmi.s    hline_or_last
hline_or_loop:    addq.l   #4,a0
                  or.w     d6,(a0)
                  dbra     d7,hline_or_loop
hline_or_last:    addq.l   #4,a0
                  move.w   d6,d7
                  and.w    d5,d7
                  or.w     d7,(a0)
...
hline_and_loop:   addq.l   #4,a0
                  and.w    d6,(a0)
                  dbra     d7,hline_and_loop
hline_and_last:   addq.l   #4,a0
                  not.w    d6
                  move.w   d6,d7
                  and.w    d5,d7
                  not.w    d7
...
;Ausgaben
;d0-d7/a1 werden zerstoert
line:             movea.l  v_bas_ad.w,a1  ;Adresse des Bilschirms
                  move.w   BYTES_LIN.w,d5 ;Bytes pro Zeile
                  tst.w    bitmap_width(a6) ;Off-Screen-Bitmap?
                  beq.s    line_laddr
                  movea.l  bitmap_addr(a6),a1 ;Adresse der Bitmap
...
line_replace:     movem.l  a0/a3,-(sp)
                  movea.w  d5,a3          ;Abstand zur naechsten Zeile
                  lea      color_map(pc),a0
                  move.b   1(a0,d7.w),d5  ;im Highbyte ist Muell!
                  movea.w  d1,a0          ;Additionswert fuer hor. Schritt
                  and.w    #3,d5
                  add.w    d5,d5
                  add.w    d5,d5
                  and.l    line_col_mask(pc,d5.w),d6 ;Maske fuer Ebenen 0/1
                  bra.s    line_rep_mask
...
line_rep_loop:    eor.l    d4,d5          ;Punkt in der Maske loeschen
                  ...
                  subq.w   #1,d0
                  bpl.s    line_rep_mask
line_rep_exit:    movem.l  (sp)+,a0/a3
                  rts
...
line_trans_tab:   DC.B line_trans0-line_trans_tab
                  DC.B line_trans1-line_trans_tab
                  DC.B line_trans2-line_trans_tab
                  DC.B line_trans3-line_trans_tab

line_trans0:      and.l    d7,(a1)+       ;0
                  rts
line_trans1:      or.l     d5,(a1)+       ;3
                  rts
...
line_replace45:   move.w   a3,d4          ;Abstand zur naechsten Zeile
                  ...
                  move.b   1(a3,d7.w),d7  ;Muell im Highbyte!
                  ext.w    d7
                  adda.w   d7,a3          ;Zeiger auf die Ausgaberoutine
...
line_rep45_xstep: add.w    d1,d3          ;e + 2dx-2dy (horizontaler Schritt)
                  ...
                  addq.l   #4,a1          ;Zeiger aufs naechste Wort
                  subq.w   #1,d0
...
line_trn45_xstep: add.w    d1,d3          ;e + 2dx-2dy (horizontaler Schritt)
                  ...
                  addq.l   #4,a1          ;naechstew Wort der Ebene
                  subq.w   #1,d0
...
scanline_loop:    movem.l  d0-d1/d3-d4,-(sp)
...
;Farbumwandlungstabelle fuer 4 Farben
...
relokation:
;Reloziert am: Sun Jan 21 20:39:22 1996

DC.w 686,68,10650,334,940,440,496,338,710,90
DC.w 524
