...
blt_inc_jmp:      move.w   r_dplanes(a6),d4
                  ...
                  tst.l    r_fgcol(a6)
                  bne.s    blt_inc_cnt
                  cmpi.b   #3,r_wmode(a6) ;ersetzend?
                  beq      copy_inc

blt_inc_cnt:      move.w   a2,r_swidth(a6)
...
blt_inc_offsets:  subq.w   #2,d1
                  ...
                  tst.w    r_splanes(a6)  ;nur eine Quellebene, expblt?
                  beq.s    blt_inc_next
...
bitblt_dec:       movea.l  r_saddr(a6),a0 ;Quelladresse
                  ...
                  lsr.w    #4,d6
                  ext.l    d6
                  lsl.l    #3,d6          ;Bytes * Anzahl Planes
...
blt_dec_cnt:      move.w   a2,r_swidth(a6)
                  move.w   a3,r_dwidth(a6)
...
fbox_mp_laddr:    sub.w    d1,d3          ;Zeilenzaehler
                  moveq    #15,d4
                  moveq    #15,d5
...
fbox_laddr:       movea.w  d4,a3          ;Bytes pro Zeile
                  muls     d1,d4
                  adda.l   d4,a1
                  moveq    #15,d4
                  moveq    #15,d5
...
fbox_white:       suba.w   d2,a3
                  subq.w   #8,a3
                  asr.w    #3,d2          ;Anzahl der Bytes pro Ebene -1
                  not.l    d4             ;Startmaske
...
fbox_usual:       move.w   a3,d0          ;Bytes pro Zeile
                  ...
                  sub.w    d2,d0
                  subq.w   #8,d0          ;- Laenge der Linie in Bytes
                  movea.w  d0,a2
                  asr.w    #3,d2          ;Anzahl der Bytes pro Ebene -1
...
fbox_replace_col: move.w   d1,-(sp)
                  ...
                  add.w    d3,d3
                  andi.w   #12,d1
                  and.l    fbox_col_mask(pc,d3.w),d6
                  and.l    fbox_col_mask(pc,d1.w),d7
...
fbox_solid:       move.w   f_color(a6),d0
                  ...
                  add.w    d1,d1
                  andi.w   #12,d0
                  move.l   fbox_scol_mask(pc,d1.w),d6
                  move.l   fbox_scol_mask(pc,d0.w),d7
...
fbox_trans_loop:  movea.l  a5,a4
                  ...
                  move.w   d2,d7
                  bmi.s    fbox_or_last
fbox_or_loop:     addq.l   #8,a4
                  or.w     d6,(a4)
                  dbra     d7,fbox_or_loop
fbox_or_last:     addq.l   #8,a4
                  move.w   d6,d7
                  and.w    d5,d7
                  or.w     d7,(a4)
...
fbox_and:         move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fbox_and_last
fbox_and_loop:    addq.l   #8,a4
                  and.w    d6,(a4)
                  dbra     d7,fbox_and_loop
fbox_and_last:    addq.l   #8,a4
                  not.w    d6
                  move.w   d6,d7
...
fline_repl:       move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fline_r_last
fline_r_loop:     addq.l   #8,a0          ;naechstes Wort der Ebene
                  move.w   d6,(a0)
                  dbra     d7,fline_r_loop
fline_r_last:     addq.l   #8,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  not.w    d7
                  and.w    d4,d7
...
fline_or:         move.w   d6,d7
                  ...
                  move.w   d2,d7
                  bmi.s    fline_or_last
fline_or_loop:    addq.l   #8,a0          ;naechstes Wort der Ebene
                  or.w     d6,(a0)
                  dbra     d7,fline_or_loop
fline_or_last:    addq.l   #8,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  and.w    d4,d7
                  or.w     d7,(a0)
...
fline_and_loop:   addq.l   #8,a0          ;naechstes Wort der Ebene
                  and.w    d6,(a0)
                  dbra     d7,fline_and_loop
fline_and_last:   addq.l   #8,a0          ;naechstes Wort der Ebene
                  swap     d4
                  not.w    d6
                  move.w   d6,d7
                  and.w    d4,d7
...
fline_eor_loop:   addq.l   #8,a0          ;naechstes Wort der Ebene
                  eor.w    d6,(a0)
                  dbra     d7,fline_eor_loop
fline_eor_last:   addq.l   #8,a0          ;naechstes Wort der Ebene
                  swap     d4
                  move.w   d6,d7
                  and.w    d4,d7
                  eor.w    d7,(a0)
...
fline:            tst.w    f_planes(a6)      ;mehrfarbiges Muster?
                  bne      fline_saddr
...
hline_replace:    moveq    #0,d0
                  ...
                  andi.w   #12,d0
                  move.l   d6,d7
                  and.l    hline_col_mask(pc,d1.w),d6
                  and.l    hline_col_mask(pc,d0.w),d7
...
hline_trans_loop: movea.l  a1,a0
                  ...
                  move.w   d2,d7
                  bmi.s    hline_or_last
hline_or_loop:    addq.l   #8,a0
                  or.w     d6,(a0)
                  dbra     d7,hline_or_loop
hline_or_last:    addq.l   #8,a0
                  move.w   d6,d7
                  and.w    d5,d7
                  or.w     d7,(a0)
...
hline_and_loop:   addq.l   #8,a0
                  and.w    d6,(a0)
                  dbra     d7,hline_and_loop
hline_and_last:   addq.l   #8,a0
                  not.w    d6
                  move.w   d6,d7
                  and.w    d5,d7
                  not.w    d7
...
line_replace:     movem.l  a0/a3,-(sp)
                  ...
                  andi.w   #12,d5
                  move.l   d6,d7
                  and.l    line_col_mask(pc,d1.w),d6 ;Maske fuer Ebenen 0/1
                  and.l    line_col_mask(pc,d5.w),d7 ;MAske fuer Ebenen 2/3
                  bra.s    line_rep_mask
...
line_trans_tab:   DC.B line_trans0-line_trans_tab
                  ...
                  DC.B line_trans4-line_trans_tab
                  DC.B line_trans5-line_trans_tab
                  DC.B line_trans6-line_trans_tab
                  DC.B line_trans7-line_trans_tab
                  DC.B line_trans8-line_trans_tab
                  DC.B line_trans9-line_trans_tab
                  DC.B line_trans10-line_trans_tab
                  DC.B line_trans11-line_trans_tab
                  DC.B line_trans12-line_trans_tab
                  DC.B line_trans13-line_trans_tab
                  DC.B line_trans14-line_trans_tab
                  DC.B line_trans15-line_trans_tab
...
line_rep45_xstep: add.w    d1,d3          ;e + 2dx-2dy (horizontaler Schritt)
                  ...
                  addq.l   #8,a1          ;Zeiger aufs naechste Wort
                  subq.w   #1,d0
...
line_trn45_xstep: add.w    d1,d3          ;e + 2dx-2dy (horizontaler Schritt)
                  ...
                  addq.l   #8,a1          ;naechstew Wort der Ebene
                  subq.w   #1,d0
...
textblt_soft:     cmpi.w   #1,wr_mode(a6) ;REPLACE oder TRANSPARENT?
                  ...
                  cmpi.w   #BLACK,t_color(a6) ;Textfarbe
                  beq.s    textblt_black
...
textblt_soft_ad:  move.l   a0,r_saddr(a6) ;Quelladresse
                  ...
                  move.w   r_planes(a6),r_dplanes(a6) ;Planes des Zielblocks - 1
