...
;******************************************************************************;
;*                                                                            *;
;*                   32768-Farb-Offscreen-Treiber fuer NVDI 4.1               *;
;*                                                                            *;
;******************************************************************************;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
header:           bra.s continue          ;Fuer Aufrufe von normale Treibern
                  ...
                  DC.L  get_extndinfo
                  DC.L  get_scrninfo
                  DC.L  get_name          ;??? should just be ptr to name
                  DC.L  0,0,0,0           ;reserviert
organisation:     DC.L  65536             ;Farben
                  DC.W  16                ;Planes
                  DC.W  2                 ;Pixelformat
...
IFNE ENABLE_040                                               ;040er-Code benutzen?
...
scrninfo:         DC.W 2                  ;Packed Pixels
                  ...
                  DC.W 16                 ;16 Ebenen
                  DC.L 65536              ;65536 Farben
                  DC.W 0,0,0
                  DC.W 5                  ;5 Bits fuer die Rot-Intensitaet
                  DC.W 6                  ;6 Bits fuer die Gruen-Intensitaet
...
scrninfo_int:     DC.W 3,4,5,6,7          ;Bits der Rot-Intensitaet
                  DCB.W 11,-1
                  DC.W 13,14,15,0,1,2     ;Bits der Gruen-Intensitaet
                  DCB.W 10,-1
...
scrninfo_mot:     DC.W 11,12,13,14,15     ;Bits der Rot-Intensitaet
                  DCB.W 11,-1
                  DC.W 5,6,7,8,9,10       ;Bits der Gruen-Intensitaet
                  DCB.W 10,-1
...
;Die Felder rgb_in_tab, rgb_calib_tab und rgb_out_tab fuer vs_color/vq_color initialisieren
;Vorgaben:
;kein Register wird veraendert
;Eingaben:
...
set_color_rgb:    lea      c_palette(a6),a1
                  ...
                  beq.s    set_col_rgb_save
                  rol.w    #8,d0          ;Intel-Format
...
get_color_rgb:    lea      c_palette(a6),a1
                  ...
                  move.w   0(a1,d0.w),d2
                  btst     #7,bitmap_flags+1(a6)   ;Intel-Format?
                  beq.s    get_color_rgbs 
                  rol.w    #8,d2          ;Motorola-Format
get_color_rgbs:   lea      rgb_out_tab(pc),a1
                  rol.w    #6,d2
                  moveq    #%111110,d0
                  and.w    d2,d0
...
set_pattern_loop: move.l   (a0)+,d0       ;32-Bit-True-Color-Fuellmuster
                  ...
                  move.w   d0,d1
                  lsr.l    #2,d1          ;Gruen-Intensitaet auf 6 Bits verkleinern
                  move.b   d0,d1
...
set_pattern_swap: move.l   (a0)+,d0       ;32-Bit-True-Color-Fuellmuster
                  move.l   d0,d1
                  lsr.l    #3,d1          ;Rot-Intensitaet auf 5 Bits verkleinern
                  move.w   d0,d1
                  lsr.l    #2,d1          ;Gruen-Intensitaet auf 6 Bits verkleinern
                  move.b   d0,d1
                  lsr.l    #3,d1          ;Blau-Intensitaet auf 5 Bits verkleinern
                  rol.w    #8,d1          ;Bytereihenfolge umkehren
                  move.w   d1,(a1)+
                  dbra     d2,set_pattern_swap
...
vr_trnfm_sdev1:   movea.l  d1,a6
                  ...
                  move.l   a1,-(sp)
                  addq.l   #1,a1          ;Ebenen 7-0 sind im Low-Byte
...
vr_trnfm_sstd1:   movea.l  d1,a6
                  ...
                  ENDM
                  addq.l   #1,a0          ;Ebenen 7-0 liegen im Low-Byte
                  bsr.s    vr_trnfm_sstd2 ;Ebenen 7-0 bearbeiten
                  movea.l  sp,a0
...
vr_trnfm_dev:     add.l    d0,d0          ;Laenge einer Ebene in Bytes
                  movem.l  d0-d1/a0-a1,-(sp)
                  addq.l   #1,a1          ;Ebenen 7-0 liegen im im Low-Byte
                  bsr.s    vr_trnfm_dev2
                  movem.l  (sp)+,d0-d1/a0-a1
                  move.l   d0,d2
                  lsl.l    #3,d2
...
fbox_universal:   move.w   wr_mode(a6),d7
                  ...
                  IFEQ OLD
                  move.w   f_interior(a6),d5
                  tst.w    f_color(a6)    ;weiss?
                  beq      fbox_solid
                  tst.w    d6             ;weisses Fuellmuster?
                  bne.s    fbox_opt
                  move.l   #$ffffffff,d7  ;weiss als Farbe benutzen
                  bra      fbox_solid

fbox_opt:         subq.w   #F_SOLID,d5    ;volles Muster?
                  beq      fbox_solid
                  subq.w   #F_PATTERN-F_SOLID,d5
                  bne.s    fbox_line_offset
                  cmpi.w   #8,f_style(a6) ;volles Muster?
                  beq      fbox_solid
                  ENDC

fbox_line_offset: moveq    #$fffffff0,d5
                  and.w    d2,d5
                  sub.w    d0,d5
                  add.w    d5,d5
                  ext.l    d6
...
fbox_repl_040:    and.w    #15,d6
                  ...
                  add.w    d0,d0
                  add.w    d6,d6
                  movea.l  fbox_repl40_stab(pc,d0.w),a2
                  movea.l  fbox_repl40_etab(pc,d6.w),a6

                  bra      fbox_repl40_mask

fbox_repl40_stab: DC.L fbox_repl40_ld0
                  DC.L fbox_repl40_sd0
                  DC.L fbox_repl40_ld1
                  DC.L fbox_repl40_sd1
                  DC.L fbox_repl40_ld4
                  DC.L fbox_repl40_sd4
                  DC.L fbox_repl40_ld5
                  DC.L fbox_repl40_sd5
                  DC.L fbox_repl40_ld6
                  DC.L fbox_repl40_sd6
                  DC.L fbox_repl40_ld7
                  DC.L fbox_repl40_sd7
                  DC.L fbox_repl40_la4
                  DC.L fbox_repl40_sa4
                  DC.L fbox_repl40_la5
                  DC.L fbox_repl40_sa5

fbox_repl40_etab: DC.L fbox_repl40_ewd0
                  DC.L fbox_repl40_ed0
                  DC.L fbox_repl40_ewd1
                  DC.L fbox_repl40_ed1
                  DC.L fbox_repl40_ewd4
                  DC.L fbox_repl40_ed4
                  DC.L fbox_repl40_ewd5
                  DC.L fbox_repl40_ed5
                  DC.L fbox_repl40_ewd6
                  DC.L fbox_repl40_ed6
                  DC.L fbox_repl40_ewd7
                  DC.L fbox_repl40_ed7
                  DC.L fbox_repl40_ewa4
                  DC.L fbox_repl40_ea4
                  DC.L fbox_repl40_ewa5
                  DC.L fbox_repl40_ea5

fbox_repl40_mask: moveq    #15,d0
                  cmp.w    d0,d3          ;wenigstens 16 Zeilen?
                  bge.s    fbox_repl40ace2
                  move.w   d3,d0

fbox_repl40ace2:  swap     d3
                  move.w   d0,d3

fbox_repl40_bloop:   swap     d3
...
fbox_repl40_line: move.w   d2,-(sp)
                  jmp      (a2)

fbox_repl40_sd0:  move.w   d0,(a1)+
                  bra.s    fbox_repl40_ld1
fbox_repl40_sd1:  move.w   d1,(a1)+
                  bra.s    fbox_repl40_ld4
fbox_repl40_sd4:  move.w   d4,(a1)+
                  bra.s    fbox_repl40_ld5
fbox_repl40_sd5:  move.w   d5,(a1)+
                  bra.s    fbox_repl40_ld6
fbox_repl40_sd6:  move.w   d6,(a1)+
                  bra.s    fbox_repl40_ld7
fbox_repl40_sd7:  move.w   d7,(a1)+
                  bra.s    fbox_repl40_la4
fbox_repl40_sa4:  move.w   a4,(a1)+
                  bra.s    fbox_repl40_la5
fbox_repl40_sa5:  move.w   a5,(a1)+
                  bra.s    fbox_repl40_lnext

fbox_repl40_ld0:  move.l   d0,(a1)+
fbox_repl40_ld1:  move.l   d1,(a1)+
fbox_repl40_ld4:  move.l   d4,(a1)+
fbox_repl40_ld5:  move.l   d5,(a1)+
fbox_repl40_ld6:  move.l   d6,(a1)+
fbox_repl40_ld7:  move.l   d7,(a1)+
fbox_repl40_la4:  move.l   a4,(a1)+
fbox_repl40_la5:  move.l   a5,(a1)+
fbox_repl40_lnext:subq.w   #1,d2
                  bmi.s    fbox_repl40_end

                  move.l   a2,-(sp)
                  lea      -32(a0),a2
                  tst.l    (a2)              ;Datencache fuellen - ist wichtig, da move16 nicht den Cache fuellt!
                  tst.l    16(a2)
                  
fbox_repl40_loop: movea.l  a2,a0
                  move16   (a0)+,(a1)+
                  move16   (a0)+,(a1)+
                  dbra     d2,fbox_repl40_loop

                  movea.l  (sp)+,a2

fbox_repl40_end:  adda.w   6(sp),a1
                  jmp      (a6)

fbox_repl40_ewa5: move.l   a5,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ea4
fbox_repl40_ewa4: move.l   a4,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed7
fbox_repl40_ewd7: move.l   d7,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed6
fbox_repl40_ewd6: move.l   d6,d2
                  swap     d2

                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed5
fbox_repl40_ewd5: move.l   d5,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed4
fbox_repl40_ewd4: move.l   d4,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed1

fbox_repl40_ewd1: move.l   d1,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_ed0
fbox_repl40_ewd0: move.l   d0,d2
                  swap     d2
                  move.w   d2,-(a1)
                  bra.s    fbox_repl40_next

fbox_repl40_ea5:  move.l   a5,-(a1)
fbox_repl40_ea4:  move.l   a4,-(a1)
fbox_repl40_ed7:  move.l   d7,-(a1)
fbox_repl40_ed6:  move.l   d6,-(a1)
fbox_repl40_ed5:  move.l   d5,-(a1)
fbox_repl40_ed4:  move.l   d4,-(a1)
fbox_repl40_ed1:  move.l   d1,-(a1)
fbox_repl40_ed0:  move.l   d0,-(a1)

fbox_repl40_next: move.w   (sp)+,d2

                  adda.l   a3,a1          ;16 Zeilen weiter
                  dbra     d3,fbox_repl40_line
                  ...
                  dbra     d3,fbox_repl40_bloop ;naechste Fuellmusterzeile
                  addq.l   #2,sp
                  rts
...
fbox_solid:       movea.w  (sp)+,a3
                  ...
                  sub.w    d6,a3 
                  sub.w    d6,a3             
                  subq.w   #2,a3                ;Abstand zur naechsten Zeile
                  cmp.w    #15,d6
                  ble      fbox_sld_short
...
fbox_solid_030:
fbox_sld_bloop30: move.w   d6,d0
                  lsr.w    #1,d0
                  bcs.s    fbox_sld_loop30
...
blt040_start_mis: not      d0
                  and.w    d6,d0          ;Anzahl der anfaenglich zu kopierenden Worte - 1
...
blt040_end:       move.w   d4,d2
                  ...
                  and.w    d6,d2          ;Anzahl der am Ende zu kopierenden Worte - 1
                  cmp.w    d6,d2
...
rgb_out_tab:      DS.W  128               ;UWORD   rgb_out_tab[3][32];
