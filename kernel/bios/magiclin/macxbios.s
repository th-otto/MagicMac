* BIOS fÅr MagiC MAC X
* ********************

* Echte Tabulatoren, Breite 5

*
* Dieses Modul enthÑlt die Atari-seitige Nachbildung des BIOS
*
...
* - Exceptionvektoren werden initialisiert, auûer priv.viol. und div0
...
* - end_os und exec_os im Variablenheader des BIOS und des AES Ñndern
*   (macht z.Zt. das MAGXBOOT, ggf. noch elegantere Methode suchen)
* - cpu_typ ($59e) initialisieren auf 40(dez) fÅr 68040 bzw. 30(dez)
*   fÅr 68030
* - Alle EintrÑge der öbergabestruktur fÅllen
*   Die Routinen fÅr Drucker und AUX bekommen ihre Parameter immer Åber
*   Register a0, das zeigt auf die Parameter auf dem Stack. Es dÅrfen keine
*   Register auûer d0-d2 und a0-a2 verÑndert werden.
* - 200Hz- Interrupt einfÅttern ($114)
* - VBL einfÅttern ($70)
*   Die Vektoren mÅssen alle Register retten.
* - TastendrÅcke und Mausdaten in den MFP-Interrupt 6 einfÅttern
*   (Vektor bei $118)
*
...
ALT_NUMKEY	EQU	1
DEADKEYS		EQU	1
COMMAND		EQU	$37			; "Apple"-Taste fÅr Calamus-UnterstÅtzung

; 0x37 ist Scancode fÅr Command (Apple)
; 0x49 ist Scancode fÅr PgUp
; 0x51 ist Scancode fÅr PgDn
; 0x4c ist Scancode fÅr AltGr
; 0x4f ist Scancode fÅr Ende
...
* Import vom DOS
	...
	XREF 	dos_init			; DOS
	XREF 	act_pd			; DOS
	XREF 	bufl_timer		; DOS (fÅr writeback)
	XREF 	secb_ext			; DOS
...
* Import vom VDI
	...
	XREF 	vdi_init			; VDI: initialisieren (fÅr MXVDI)
...
; BPB, erweitert fÅr Floppy Disk
fbpb_bpb:      DS.B $12  /*   0: BPB */
fbpb_ntracks:  DS.W 1    /* $12: AnzahlTracks */
fbpb_nsides:   DS.W 1    /* $14: NSIDES */
fbpb_nst:      DS.W 1    /* $16: NSIDES*SPT */
fbpb_spt:      DS.W 1    /* $18: SPT */
fbpb_nhid:     DS.W 1    /* $1a: NHID */
fbpb_serial:   DS.L 1    /* $1c: charserial[3] */
fbpb_msserial: DS.L 1    /* $20: charmsserial[4] */
fbpb_sizeof:
...
* ## unbenutzt. Ab <clear_area> werden beim Warmstart 64 kB gelîscht
...
bpbx:               DS.B 32*fbpb_sizeof /* BPBs fÅr 32 Laufwerke      */
p_iorec_ser1:		DS.L 1			/* IOREC *p_iorec_ser1		*/
; IOREC:			DS.B $e			/*	0: IOREC fÅr Eingabe	*/
;				DS.B $e			/*  $e: IOREC fÅr Ausgabe	*/
...
altgr_status:		DS.B 1			/* char altgr_status		*/
	IF	ALT_NUMKEY
alt_numkey:		DS.B	1			/* FÅr Alt-Num0..Num9		*/
	ENDIF
	IF	DEADKEYS
deadkey_asc:		DS.B	1			/* FÅr "dead keys"			*/
deadkey_scan:		DS.B	1			/* FÅr "dead keys"			*/
deadkey_kbsh:		DS.B	1			/* FÅr "dead keys"			*/
	EVEN
deadkey_subtab:	DS.L	1			/* FÅr "dead keys"			*/
	ENDIF
	EVEN
...
pkill_vector:		DS.L	1			/* 24: VERKETTEN: z.B. fÅr DSP	*/
...
ttram_md: 		DS.L 4			/* MD fÅr TTRAM-Block			*/
	IFNE FALCON
scrbuf_adr:		DS.L 1			/* Startadresse (netto, ohne MCB)	*/
scrbuf_len:		DS.L 1			/* tatsÑchliche LÑnge			*/
	ENDIF
pe_slice: 		DS.W 1			/* fÅr XAES (Zeitscheibe)		*/
pe_timer: 		DS.W 1			/* fÅr XAES (Zeitscheibe)		*/
first_sem:
...
p_mgxinf:
pgm_userst:		DS.L 1
dflt_maptable: 	DS.L 4*6			/* fÅr 4 EintrÑge Ö 24 Bytes		*/
intern_maptab: 	DS.L 4*6		    ;interne MapTab. EnthÑlt die Adressen
							    ;der seriellen Mag!X-Biosroutinen
;
; Anschlieûend eine Kopie der GerÑtevektoren.
; Ist der Eintrag 0, so ist eine externe Routine eingehÑngt, andernfalls ist
...
; auch fÅr andere Devices nÅtzlich, die sich an die Registerkonventionen des
...
;Bconoutvec:		DS.L 8
do_gettime:		DS.W 1
warmbvec: 		DS.L 1			/* Sprungadr. fÅr Ctrl-Alt-Del	*/
coldbvec: 		DS.L 1			/* Sprungadr. fÅr Ctrl-Alt-Rsh-Del */
sust_len:			DS.L	1			/* Supervisorstack pro Applikation */
datemode:			DS.W	1		;fÅr date2str (->STD.S)
log_fd:			DS.L	1			/* DateiHandle fÅr Bootlog */
log_fd_pd:		DS.L	1			/* Prozeûdeskriptor fÅr Handle */
log_oldconout:		DS.L	1			/* Alter Vektor fÅr Bootlog */
p_vt52:			DS.L	1			/* fÅr VT52.PRG */
__e_bios:
...
IF __e_bios_ovfl >= $840
 "öberlauf der öberlauf-Bios-Variablen"
ENDIF
...
**********************************************************************
*
* Header fÅr MagicMacX. öbergabestruktur zwischen
* Atari und MacOS X
*
**********************************************************************
...
MSysX:
 ...
 DC.L	tab_unshift		; 5*128 Bytes fÅr Tastaturtabellen
 DC.L	mem_root
 DC.L	act_pd
 DC.L	act_appl
 ...
 DC.L	0				; MacSys_Daemon
 DC.L     0                   ; MacSys_BlockDev
 DC.L     0                   ; reserved for future
 DC.L     0                   ; reserved for future
 DC.L	0				; MacSys_Yield

**********************************************************************
*
* Alter Header fÅr MagicMac. Leider notwendig, weil die Behnes
* ihr VDI nicht angepaût haben.
*
**********************************************************************
...
MSys:
 ...
 DC.L	tab_unshift		; $08 5*128 Bytes fÅr Tastaturtabellen
 DC.L	0				; $0c Version
 ...
 DC.L	0				; $40 prt_cis 	FÅr Drucker (Atari -> MAC)
 DC.L	0				; $44 prt_cos
 DC.L	0				; $48 prt_cin
 DC.L	0				; $4c prt_cout
 DC.L	0				; $50 ser_rsconf
 DC.L	0				; $54 ser_cis 	För ser1 (Atari -> MAC)
 DC.L	0				; $58 ser_cos
 DC.L	0				; $5c ser_cin
 DC.L	0				; $60 ser_cout
 ...
 DC.L	BehneError		; $78 MacSys_error. ###### FöR BEHNES ######
 DC.L	0				; MacSys_init
...
bot_ver_ok:
 ...
 frestore long_zero 			; fÅr 68882

bot_ok1:
* BIOS- Variablenbereich lîschen
 lea 	clear_area,a0
 lea 	__e_dos,a1
 moveq	#0,d0
bot_vclear:
 ...
 lea		config_status,a0		; config-Status-Block lîschen
 moveq	#7-1,d0
ccfl_loop:
 clr.l	(a0)+
 dbra	d0,ccfl_loop
...
* Installation einiger Exceptionvektoren fÅr Disk und Ausgabe

 move.l   #host_hdv_init,hdv_init
 move.l   #host_hdv_rwabs,hdv_rw
 move.l   #host_hdv_getbpb,hdv_bpb
 move.l   #host_hdv_mediach,hdv_mediach
 move.l   #host_hdv_boot,hdv_boot
...
; clr.l	_drvbits				; werden vom Emulator gesetzt
 clr.l	_shell_p				; !!! wird jetzt gelîscht
 move.l	#hdl_pling,bell_hook	; Ton fÅr ^G
 move.l	#hdl_klick,kcl_hook 	; Tastenklickroutine
 move.l	#warm_boot,warmbvec 	; Sprungvektor fÅr Ctrl-Alt-Del
 move.l	#cold_boot,coldbvec 	; Sprungvektor fÅr Ctrl-Alt-Rshift-Del
...
* unbenutzte oder Bomben- Exceptionvektoren initialisieren
 ...
 moveq    #64-3,d0				; 0 und 1 unverÑndert, 2..63 auf Bomben
bot_loop:
 move.l   a1,(a0)+
 adda.l   d1,a1
 dbf      d0,bot_loop
...
* etv_timer und etv_term auf RTS, VBL- Queue lîschen
...
* Devicevektoren initialisieren
 ...
 lea 	ori_dev_vecs(pc),a0 	; ROM-GerÑtevektoren
 lea 	dev_vecs,a1
 lea 	mbiosvecs,a2
 moveq	#$1f,d0
bot_loop4:
...
* MFP und Vektoren initialisieren ( bsr init_mfp )
 ...
 lea 	int_hz_200(pc),a2		; der 200Hz- ZÑhler ...
...
* DOS initialisieren, Supervisorstack und Diskpuffer anlegen
 ...
 move.l	#SUPERSTACKLEN,sust_len	; Grîûe des Supervisorstacks pro App
 bsr 	dos_init
 ...
 beq 	fatal_err
 move.l	d0,app0
 add.l	(sp)+,d0				; StacklÑnge addieren
 addq.l	#2,sp
...
;bsr 	exec_respgms			; residente Programme ausfÅhren
...
* Wird Åber den MgMx-Cookie aufgerufen.
*
...
*********************************************************************
*
* Der Zugriff bei Rwabs ist bei gesetztem LOCK nur fÅr den
* sperrenden Prozeû erlaubt
*

IRwabs:
 movem.l	d3-d7/a3-a6,-(sp)
 subq.l	#4,sp			; Platz fÅr Zeiger
 lea 	12(a0),a0
 move.l	(a0),-(sp)		; lrecno
...
* Auf LOCK testen und Zugriffszeit merken
 ...
 add.w	a1,a1
 add.w	a1,a1
 move.l	a1,4(sp)			; merken fÅr DOS- Writeback
 move.l	dlockx(a1),d3
 beq.b	rwabs_ok
 cmp.l	act_pd,d3
 bne.b	rwabs_elocked
rwabs_ok:
 clr.l	bufl_timer(a1) 	; fÅr DOS- Writeback (als in Arbeit markieren)
...
rwabs_ende:
 move.l	(sp)+,a1
 move.l	_hz_200,bufl_timer(a1)	; fÅr DOS- Writeback
...
dummynopfn:
 nop 	  ;EingefÅgt wegen jsr 4(a2) in Bconout()!
 nop
dummyfn:
 rts
...
* TOS 3.06 bearbeitet zunÑchst die VBL-Queue
*

int_vbl:
 addq.l	#1,_frclock			; Anzahl aller VBLs mitzÑhlen
 subq.w	#1,vblsem 			; VBL gesperrt ?
 bmi 	ivbl_locked			; ja, ende
 movem.l	d0/d1/d2/d3/d4/d5/d6/d7/a0/a1/a2/a3/a4/a5/a6,-(sp)
 addq.l	#1,_vbclock			; Anzahl aller VBL-Routinen mitzÑhlen
...
*	wird der nÑchste Parameter abgefragt. Wenn er 'iocl' ist, dann
*	folgen Bios-GerÑtenummer, Kommando und Puffer (von Fcntl).
*
...
; ssp => Um dies Stackbelastung mîglichst wenig zu halten, sollten kurze
...
Supexec:
   move.l   (a0),a0
   movem.l  d3-d7/a3-a6,-(sp) ;aus KompatibilitÑtsgrÅnden sichern
   jsr	  (a0)
   movem.l  (sp)+,d3-d7/a3-a6
   rte
...
calc_Bvec:
   ...
   subq.w   #6,d0			;Offset fÅr erweiterte serielle Schnittstellen
   cmp.w	  bconmap_struct+4,d0 ;Anzahl zusÑtzlicher Schnittstellen
   bcc	  ret0_rte		;Åberschritten, return(0L) (kein Fehler??!!??)

   lea	  intern_maptab,a2	;max 4 * 6 EintrÑge auf die seriellen-Routinen
   ...
   movea.l  (a2),a2
   cmpa.l   a1,a2			;id. ?
   bne.b	  Bvec_changed 	;andere Routine eingehÑngt !
   jsr	  (a2)
   rte
...
Bconout:
   ...
   movea.l  (a2),a2
   cmpa.l   a1,a2			;id. ?
   bne.b	  Bovec_changed	;andere Routine eingehÑngt !
   jsr	  4(a2)			; "lea 6(sp),a0" Åberspringen
   rte
...
Getmpb:
 ...
 move.l	d0,(a1)+				; LÑnge
 clr.l	(a1) 				; kein owner
* ggf. zweiten MD fÅr TT-RAM einrichten
 cmpi.l	#$1357bd13,ramvalid	; TT-RAM gÅltig ?
...
*			anzulegen. RÅckgabe ENSMEM, falls dies nicht geht.
*			Legt keinen (!) Cookie _FRB an.
*			Ist auf dem Mac ÅberflÅssig
...
Setexc:
 movea.w	(a0)+,a1				; nr
 add.w	a1,a1
 add.w	a1,a1				; mal 4 fÅr Langwortzugriff
...
**********************************************************************
*
* global void debug_puts(a0 = char *errmsg)
*
...
* void fatal_err( void )
*
* Ein fataler Fehler fÅhrt zum Anhalten des Systems
*
...
*
* FÅhrt gerÑtespezifische Aktionen aus.
*
...
* Unterfunktion 0: Prtblk einklinken
 move.l	prtblk_vec,d0			; alte Routine zurÅckgeben
...
_Scrdmp:
 ...
 jsr 	(a0)
* MagiC 3.0: DUMMY-ROUTINE FöR HARDCOPY
do_hardcopy:
_scrd_err:
 move.w	#-1,_dumpflg
 rts
...
crsh_loop:
 ...
 dbf 	d0,crsh_loop
 move.w	jmpop(pc),-6(a1)			; Opcode fÅr "jmp"
...
_Bconmap:
 ...
 beq 	_bmp_ende 		; ja, ende
 moveq	#0,d0			; RÅckgabe: Fehler
 subq.w	#6,d1			; Nummer um Offset 6 dekrementieren
 bmi 	_bmp_ende 		; Wert war < 6, return(0)
 cmp.w	bconmap_struct+4,d1 ; TabellenlÑnge
 bcc 	_bmp_ende 		; Wert war >= TabellenlÑnge, return(0)
 move.w	sr,-(sp)
 ori.w	#$700,sr			; im TOS 2.05 vergessen!
* aktuellen Port in maptab retten (wozu, er kommt doch daher ?)
 move.w	bconmap_struct+6,d0 ; aktueller Port, diesen zurÅckgeben
 ...
 move.l	p_rsconf,(a0)+ 	; Rsconf fÅr dev 1
 move.l	p_iorec,(a0)+		; iorec  fÅr dev 1
...
init_bconmap:
 ...
 moveq	#1,d0			; zunÑchst eine Schnittstelle
 move.w	d0,(a1)+			; LÑnge festlegen
...
ins_internmap: 			;Maptab der Mag!X-eigenen Routinen erstellen
 ...
 lea 	dflt_maptable,a0
 move.w	sr,-(sp)
 ori.w	#$700,sr			;in GerÑtevektoren eintragen...
...
maptab_data:
 ...
 DC.L p_iorec_ser1		; iorec fÅr ST-MFP
...
bconin_con:
 ...
 moveq	#4,d2				; Grîûe eines Arrayelementes
_bconin:
 lea 	8(a0),a1				; *Tail
...
* Das Zeichen ist immer noch da
 add.w	d2,d1				; Head-Index erhîhen
 movea.l	(a0)+,a1				; Pufferzeiger
 cmp.w	(a0)+,d1				; mit Puffergrîûe vergleichen
 bcs.b	bin_l1
...
ori_iorec_kb:
 ...
 DC.W	$0100				; Grîûe 256 Bytes = 64 Langworte
 DC.W	0					; Head Index
 DC.W	0					; Tail Index
...
_mfpint:
 ...
 add.w	d0,a0
 add.w	d0,a0
 add.w	d0,a0				; * 4 fÅr Langwortzugriff
...
Dosound:
 ...
 bmi.b	dsnd_get				;  ist -1 => laufendes zurÅckgeben
 move.l	d1,(a1)+				; neues setzen
 clr.b	(a1) 				; Flag lîschen
dsnd_get:
 rte
...
; eori.w	 #$300,sr 			 ; von IPL 6 auf 5 schalten
 ...
 lea 	keyrepeat,a0
 move.b	(a0)+,d0				; Scancode, Taste gedrÅckt ?
 beq.b	i200_nokey			; nein
 subq.b	#1,(a0)				; ZÑhler fÅr Verzîgerung dekrementieren
 bne.b	i200_nokey			; ist nicht 0
 move.b	key_reprate,(a0)		; ZÑhler neu laden
 lea 	iorec_kb,a0			; Taste <d0> in Tastaturpuffer ...
 bsr 	keyrep_entry			; ... eintragen
i200_nokey:
 move.w	_timer_ms,-(sp)
 movea.l	etv_timer,a0
 jsr 	(a0) 				; Userinterruptroutinen ausfÅhren
 addq.w	#2,sp
 movem.l	(sp)+,a6/a5/a4/a3/a2/a1/a0/d7/d6/d5/d4/d3/d2/d1/d0
i200_yield:
 tst.w	pe_slice
...
Setscreen:
 ...
 moveq	#-1,d1
 cmp.l	d0,d1
 beq.b	setscr_nolog		    ; log. Adr. nicht Ñndern
 move.l	d0,_v_bas_ad
setscr_nolog:
 rte
...
Kbrate:
 ...
 bmi.b	kbr_ende				; ist -1, keine énderung
...
Keytbl:
 ...
 move.l	a1,d0				; RÅckgabewert
 moveq	#3-1,d2				; ZÑhler fÅr 3 DurchlÑufe
ktbl_loop:
 move.l	(a0)+,d1
 bmi.b	ktbl_noset
 move.l	d1,(a1)
...
* in ein ASCII-Zeichen (in Groûbuchstaben).
...
altcode_asc:
 ...
 cmpi.w	#$78,d0
 bcs.b	ala_nonum
* Sonderbehandlung fÅr Alt-1 bis Alt-'
 cmpi.w	#$83,d0
 bhi.b	ala_nix
...
* In MagiC drei weitere (fÅr AltGr)
* IN MagiC ab 1.6.97 nochmals 3 weitere (fÅr Alt)
*
...
**********************************************************************
*
* "Interrupt" fÅr MIDI und Keyboard (MFP- Interrupt 6).
*
* Der Interrupt wird hier nur simuliert, und zwar folgendermaûen:
*
* d7 = 'MAGC': Daten sind gÅltig
*
* dann:	d6 = 0:	MIDI
*		d6 = 1:	Tastatur
*			d5 = Atari-Scancode
...
no_key:
   ...
   move.b   d0,d2
   subi.b   #$f6,d2                 ;d2 enthÑlt Paketnummer 0..9
...
; 6=$fe: Joystick 0
; 7=$ff: Joystick 1
pk_code_len:         ;in der Form: ikbd_state, Paket-LÑnge
 dc.b 1,7
 dc.b 2,5
 dc.b 3,2
 dc.b 3,2
...
handle_package:
   ...
   ext.w    d1                      ;muû fÅr Indizierung auf .w gebracht werden
   add.w    d1,d1                   ;d1 = ikbd_state (1,2,3,4,5)
   add.w    d1,d1
   add.w    d1,d1                   ;* 8
   lea      lblFC3C1A-8(pc,d1.w),a2 ;ikbd_state auf 0..4 umrechnen
   movea.l  (a2)+,a0                ;Paketanfang
   move.w   (a2)+,d1                ;PaketlÑnge
   ...
   bne.b    lblFC3C18               ;noch nicht fertig
lblFC3C0E:
                                    ;aus Komp.grÅnden bleibt d0 = (a1).b
   move.l   a0,-(sp)                ;Paketanfang als Parameter

; Åbler Patch fÅrs VDI, das hier an der Interrupt-Maske herummanipuliert
...
handle_joy:
   ...
   bra.b    lblFC3C0E               ;Vektor anspringen
;--------------------------------------------------
lblFC3C1A:
 DC.L     pack_state
 DC.W     7
...
;   bsr.b    _handlekey
;   eori.w   #$300,sr          ;von IPL 5 auf 6 zurÅck
;   rts
...
* Systemfehler provoziert hat. FÅhrt einen Warmstart aus.
*
...
;  reset						 ;versuchsweise
   ...
   bne	  sys_start
   move.l   d0,a0
   lea	  warmb_00(pc),a6		 ;RÅcksprungadresse
   jmp	  (a0)
   bra	  sys_start
...
tab_altgr:
*	   $0		    $4			$8			 $c
 DC.B  $00,YYY,$fc,$fd,$fe,$fb,$ae,$ee,$7b,'[',']',$7d,'\',$f4,YYY,YYY  * $0
...
tab_dead:
 DC.B	'∫','`','^',0
 DC.B	'a','†','e','Ç','E','ê','i','°','o','¢','u','£',0
 DC.B	'a','Ö','A','∂','e','ä','i','ç','o','ï','u','ó',0
 DC.B	'a','É','e','à','i','å','o','ì','u','ñ',0,0
...
* _CPU Cookie, Loword enthÑlt den <cpu_typ>
 move.l	#'_CPU',(a5)+
 clr.w	(a5)+
 move.w	cpu_typ,(a5)+
...
* Endmarkierung
 clr.l	(a5)+
 move.l	#NCOOKIES,(a5) 		; Platz fÅr insgesamt 20+1 Cookies
 move.l	(sp)+,a5
 rts
...
* Gibt die Anzahl der ausgegebenen Zeichen zurÅck.
*

prn_wrts:
 cmpi.l	#bconout_prt,$57e		; bconout fÅr PRT
 bne.b	old_prn_wrts			; hat sich geÑndert !!!
 move.l	d0,-(sp)
 move.l	a0,-(sp)
 lea		(sp),a1				; Param: Zeiger auf Struktur (Adr/LÑnge)
...
*
* Wie prn_wrts, geht aber Åber BIOS
*
...
old_prn_wrts:
 ...
 moveq	#3,d6
 swap	d6			 			; Fcode Bconout/BIOS- GerÑt
 move.l	d0,d7
 bra.b	bpw_nextloop
bpw_loop:
 moveq	#0,d0
 ...
 trap	#$d
 addq.l	#6,sp
 tst.w	d0						; bei PRN Status prÅfen !
