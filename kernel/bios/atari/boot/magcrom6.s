...
no_double:
        ...
        add.l   #$00040000,d0
        move.l  d0,buffer_end
        movea.l found_magic_name,a0
        bsr     loadfile
        bmi     errexit
        move.l  d0,d6
        lea     load_err_msg,a0
        cmp.l   found_magic_size,d6
        bne     errexit
        cmpi.w  #$601A,(a6)
        bne     exit
...
not_tt:
        ...
        movea.l tosstart,a5
        .dc.w 0xdbfc,0x0000,size_of_romcode  /* adda.l  #0x2b3c,a5 */
        movea.l a5,a1
        move.l  a1,start_of_magic_rom
        lea     buffer,a0
        bsr     relocate
        move.l  a0,end_of_magic
        move.l  d1,magic_reloc_size
        move.l  d0,d5
        move.l  d0,magic_ram_size
        add.l   start_of_magic_rom,d0
        move.l  d0,end_of_code_rom
...
/*
 * copy magic.ram to rom_buffer
 */
        lea     buffer,a0
        lea     sizeof_PH(a0),a0
        lea     size_of_romcode,a1
        adda.l  rom_buffer,a1
        move.l  a1,start_of_magic
        move.l  magic_ram_size,d0
        bsr     memcpy
        movea.l end_of_magic,a0
        move.l  magic_reloc_size,d0
        bsr     memcpy
        move.l  a1,start_of_scsi

        move.l  end_of_code_rom,d0
        addq.l  #1,d0
        and.l   #$FFFFFFFE,d0
        move.l  d0,start_of_reloc_magic_rom
        add.l   magic_reloc_size,d0
        addq.l  #1,d0
        and.l   #$FFFFFFFE,d0
        move.l  d0,end_of_code_rom
        ...
        tst.w   scsi_opt
        beq     no_scsi_ram
        lea     scsi_ram_path,a0
        bsr     loadfile
        bpl.s   scsi_ram_ok
        lea     scsi_ram_path+3,a0
        bsr     loadfile
        bmi     no_scsi_ram
scsi_ram_ok:
        move.l  d0,d6
        lea     buffer,a0
        movea.l end_of_code_rom,a1
        move.l  a1,start_of_scsi_rom
        bsr     relocate
        addq.l  #1,d0
        and.l   #$FFFFFFFE,d0
        adda.l  d0,a1
        move.l  a1,start_of_reloc_scsi_rom
        move.l  d1,scsi_reloc_size
        addq.l  #1,d1
        and.l   #$FFFFFFFE,d0
        adda.l  d1,a1
        move.l  a1,end_of_code_rom
        move.l  a0,-(a7)
        lea     buffer,a0
        move.l  10(a0),d1
        add.l   d0,d1
        move.l  d1,scsi_ram_size
        move.l  d0,scsi_rom_size
        lea     buffer+sizeof_PH,a0
        movea.l start_of_scsi,a1
        bsr     memcpy
        movem.l start_of_scsi,a0
        bsr     find_scsi_version
        movea.l (a7)+,a0
        move.l  scsi_reloc_size,d0
        bsr     memcpy
        move.l  a1,start_of_scsi
        movea.l start_of_magic,a0
        move.l  scsi_ram_size,d0
        add.l   d0,12(a0)
        move.l  20(a0),d1
        sub.l   start_of_magic_rom,d1
        adda.l  d1,a0
        cmpi.l  #$87654321,(a0)+
        bne.s   scsiram1
        move.l  (a0),endofvars
...
find_magic_ram:
        ...
        bpl.s   magic_found
find_magic_end:
        movem.l (a7)+,d6         /* BUG: should be a6 */
        tst.w   found_magic_name /* BUG: should be test.l */
        rts
magic_found:
        move.l  a6,found_magic_name
        move.l  d0,found_magic_size
        cmp.l   #$0002C000,d0
        bcs.s   find_magic_end
...
format_version1:
        ...
        move.w  d1,d0
        bsr     outhex
        lea     magic_version_string+16,a0
        move.w  24(a1),d1
        move.w  d1,d0
        ror.w   #4,d0
        bsr     outhex
...
format_version2:
        ...
        move.w  d1,d0
        bsr     outhex
        dbf     d2,format_version2
        lea     magic_version_string,a0
        bsr     cconws
        movem.l (a7)+,d0-d2/a0
        rts
...
getfilesize:
        ...
        rts
getfilesize1:
        lea     file_not_found_err,a0
        lea     file_not_found_err+6,a1
getfilesize2:
        move.b  (a5)+,(a1)+
        bne.s   getfilesize2
        move.b  #' ',-1(a1)     /* BUG: will copy the longer filename first */
        moveq   #-1,d0
        movem.l (a7)+,d1-d7/a1-a6
        rts
...
loadfile:
        ...
        rts
loadfile_err:
        lea     loadfile_errmsg,a0
        lea     loadfile_errmsg+6,a1
loadfile_err1:
        move.b  (a5)+,(a1)+
        bne.s   loadfile_err1
        move.b  #' ',-1(a1)
...
/*
 * input:
 *   a0: program file header
 *   a1: address to relocate to
 *   d6: length of program file
 * output:
 *   d0: size of text+data
 *   d1: length of relocations
 *   a0: end of text+data
 */
relocate:
...
memcheck13:
        ...
        move.l  end_of_code_rom(pc),fstrm_beg
        tst.l   ramtop
        bne.s   memcheck15
...
/*
 * F3: toggle load of MagiC to RAM
 */
handle_key_f3:
        tst.l   ramtop
        bne.s   handle_key_f3_1
        move.w  config_status,d0
        bset    #0,d0
        bra.s   new_config
handle_key_f3_1:
...
menu3:
        ...
        move.l  start_of_magic_rom(pc),resvector
        move.w  old_palmode(pc),palmode
        moveq   #0,d7
        move.w  config_status,d0
        btst    #0,d0
        bne.s   menu5
        movea.l start_of_magic_rom(pc),a0
        move.l  start_of_magic_ram(pc),d2
        move.l  magic_ram_size(pc),d6
        btst    #2,d0
        beq.s   menu4
        add.l   scsi_ram_size(pc),d6
menu4:
        add.l   d6,d2
        and.l   memmask(pc),d2
        move.l  d2,fstrm_beg
        ...
        move.l  d2,resvector
        movea.l d2,a1
        movea.l start_of_reloc_magic_rom(pc),a2
        move.l  magic_reloc_size(pc),d5
        bsr     rom2ram
menu5:
        move.w  config_status,d0
        btst    #2,d0
        beq.s   menu7
        move.l  start_of_scsi_rom(pc),d0
        beq.s   menu7
        movea.l fstrm_beg,a1
        cmpa.l  ramtop,a1
        movea.l endofvars(pc),a1
        move.l  a1,d0
        beq.s   menu7
        adda.l  scsi_ram_size(pc),a1
        bra.s   menu6

        cmpa.l  #FASTRAM_START,a1
        bne.s   menu6
        move.l  start_of_magic_ram(pc),d2
        add.l   scsi_ram_size(pc),d2
        and.l   memmask(pc),d2
        movea.l d2,a1
        move.l  d2,fstrm_beg
menu6:
        movea.l start_of_scsi_rom(pc),a0
        move.l  scsi_rom_size(pc),d6
        suba.l  scsi_ram_size(pc),a1
        move.l  a1,d7
        movea.l start_of_reloc_scsi_rom(pc),a2
        move.l  scsi_reloc_size(pc),d5
        bsr     rom2ram
menu7:
        movea.w #$0600,a0
        moveq   #0,d0
        move.w  #$0FFF,d1
        move.l  endofvars(pc),d2
        sub.l   #18,d2
menu8:
        move.l  d0,(a0)+
        move.l  d0,(a0)+
...
check_boot_pref1:
        tst.l   ramtop
        bne.s   check_boot_pref2
        bset    #0,d0
check_boot_pref2:
        move.l  start_of_scsi_rom(pc),d1
        bne.s   check_boot_pref3
        bclr    #2,d0
check_boot_pref3:
        move.w  d0,config_status
        movem.l (a7)+,d0-d2/a0-a2
...
printmenu2:
        ...
        move.l  start_of_scsi_rom(pc),d1
        beq.s   printmenu5
        lea     f4_noscsi_msg(pc),a0
        btst    #2,d0
        beq.s   printmenu3
        lea     f4_scsi_msg(pc),a0
printmenu3:
        bsr     printstr
        movem.l (a7)+,d0-d1/a0
        rts
...
bootversionstring:
        ...
        dc.b    '(F2)    TOS ',0
scsi_version_msg:
        dc.b    'SCSI-TREIBER V?.??',0
f4_scsi_msg:
        dc.b    '(F4) SCSI WIRD INSTALLIERT      ',0
f4_noscsi_msg:
        dc.b    '(F4) SCSI WIRD NICHT INSTALLIERT',0
spaces:
        dc.b    '                                ',0
f3strrom:
        dc.b    '(F3) MAGIC! LAEUFT IM ROM',0
f3strram:
        dc.b    '(F3) MAGIC! LAEUFT IM RAM',0
        dc.b    'ncc1701d'
start_of_magic_ram:
        dc.l    FASTRAM_START+0x8000
memmask:
        dc.l    $FFFF8000
end_of_code_rom:
        dc.l    0
...
led_blink:
        dc.w    0
tos_rom_addr:  dc.l    0x00E00000

        dc.b    'Wilfried Mintrop 30.9.1994'
        dc.l    0
...
end_of_romcode:
size_of_romcode equ end_of_romcode-start_of_romcode
...
scci_ram_msg:
        dc.b    'SCSI.RAM wurde erfolgreich mit eingebunden',$0D,$0A,0
magic_version_string:
        dc.b    'MagiC V0.00 vom 00.00.0000',$0D,$0A,0
eprom_filename:
        dc.b    'mag_fc_e.032',0
magixrom_img_name:
        dc.b    'magixrom.img',0
...
file_not_found_err:
        dc.b    'Datei               nicht gefunden',$0D,$0A,0
loadfile_errmsg:
        dc.b    'Datei               konnte nicht geladen werden',$0D,$0A,0
load_err_msg:
        dc.b    'Falsche Bytezahl geladen',$0D,$0A,0
magix_too_large_err:
