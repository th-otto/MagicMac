...
void obj_malen(int whdl, OBJECT *tree, int index)
{
	...
	index = 0;
	wind_get(whdl,WF_FIRSTXYWH,&list.g_x,&list.g_y,&list.g_w,&list.g_h);
	do	{
		if	(rc_intersect(&neu,&list))
			objc_draw(tree,index,1,list.g_x,list.g_y,list.g_w,list.g_h);
		wind_get(whdl,WF_NEXTXYWH,&list.g_x,&list.g_y,&list.g_w,&list.g_h);
		}
	while(list.g_w > 0);
}
	...
	do	{
		if	(rc_intersect(&scr, &g))
			...
			if	(xcopy > 0)
				...
				clip_rect[0] = scrx;
				clip_rect[1] = scry;
				clip_rect[2] = scrx+scrw-1;
				clip_rect[3] = scry+scrh-1;
...
{
	wind_open(w->handle, w->out.g_x,
			w->out.g_y, w->out.g_w, w->out.g_h);
	arrange_window(w);
}
...
{
	objc_draw(w->tree,0,1,g->g_x,g->g_y,g->g_w,g->g_h);
}
